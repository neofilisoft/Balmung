...
namespace BalmungEditor
{
    ...
    public static class PluginUtils
    {
        ...
        public static Texture TryGetPluginIcon(Plugin plugin)
        {
            ...
            var iconPath = Path.Combine(assemblyDir, assemblyName + ".Icon.balmung");
            if (!File.Exists(iconPath))
            {
                // Try path relative to the plugin project Content
                iconPath = Path.Combine(assemblyDir, "../../../../../Content", assemblyName + ".Icon.balmung");
                if (!File.Exists(iconPath))
                {
                    return null;
                }
            ...
            return BalmungEngine.Content.LoadAsync<Texture>(iconPath);
        }
