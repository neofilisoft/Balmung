...
using BalmungEditor.GUI.ContextMenu;
using BalmungEditor.GUI.Input;
using BalmungEditor.GUI.Tree;
using BalmungEditor.SceneGraph;
using FlaxEngine;
...
namespace BalmungEditor.Utilities
{
    ...
    public static class Utils
    {
        ...
        /// </summary>
        public static readonly string FlaxEngineAssemblyName = "BalmungEngine.CSharp";
        ...
        public static string FormatBytesCount(ulong bytes)
        {
            ...
            if (order >= 3) // GB or higher use up to 2 decimal places for more precision
                return string.Format("{0:0.##} {1}", BalmungEngine.Utils.RoundTo2DecimalPlaces(bytesPrev / 1024.0f), MemorySizePostfixes[order]);
        ...
        internal static object CloneValue(object value)
        {
            ...
            if (value is BalmungEngine.Object)
                return value;
        ...
        internal static void StructureToByteArray(object value, int valueSize, IntPtr tempBuffer, byte[] dataBuffer)
        {
            ...
            if (valueType.IsEnum)
            {
                var ptr = BalmungEngine.Interop.NativeInterop.ValueTypeUnboxer.GetPointer(value, valueType);
                BalmungEngine.Utils.MemoryCopy(tempBuffer, ptr, (ulong)valueSize);
            }
            else
        ...
        {
            if (valueType.IsEnum)
                return BalmungEngine.Interop.NativeInterop.MarshalToManaged(valuePtr, valueType);
        ...
        private static string InternalFormat(double value, string format, BalmungEngine.Utils.ValueCategory category)
        {
            switch (category)
            {
            case BalmungEngine.Utils.ValueCategory.Distance:
            ...
            case BalmungEngine.Utils.ValueCategory.Angle: return value.ToString(format, CultureInfo.InvariantCulture) + "Â°";
            case BalmungEngine.Utils.ValueCategory.Time: return value.ToString(format, CultureInfo.InvariantCulture) + Units.Unit("s");
            // some fonts have a symbol for that: "\u33A7"
            case BalmungEngine.Utils.ValueCategory.Speed: return (value / Units.Meters2Units).ToString(format, CultureInfo.InvariantCulture) + Units.Unit("m/s");
            case BalmungEngine.Utils.ValueCategory.Acceleration: return (value / Units.Meters2Units).ToString(format, CultureInfo.InvariantCulture) + Units.Unit("m/sÂ²");
            case BalmungEngine.Utils.ValueCategory.Area: return (value / Units.Meters2Units / Units.Meters2Units).ToString(format, CultureInfo.InvariantCulture) + Units.Unit("mÂ²");
            case BalmungEngine.Utils.ValueCategory.Volume: return (value / Units.Meters2Units / Units.Meters2Units / Units.Meters2Units).ToString(format, CultureInfo.InvariantCulture) + Units.Unit("mÂ³");
            case BalmungEngine.Utils.ValueCategory.Mass: return value.ToString(format, CultureInfo.InvariantCulture) + Units.Unit("kg");
            case BalmungEngine.Utils.ValueCategory.Force: return (value / Units.Meters2Units).ToString(format, CultureInfo.InvariantCulture) + Units.Unit("N");
            case BalmungEngine.Utils.ValueCategory.Torque: return (value / Units.Meters2Units / Units.Meters2Units).ToString(format, CultureInfo.InvariantCulture) + Units.Unit("Nm");
            case BalmungEngine.Utils.ValueCategory.None:
        ...
        public static string FormatFloat(float value, BalmungEngine.Utils.ValueCategory category)
        {
            if (float.IsPositiveInfinity(value) || value == float.MaxValue)
                return "Infinity";
            if (float.IsNegativeInfinity(value) || value == float.MinValue)
                return "-Infinity";
            if (!Units.UseUnitsFormatting || category == BalmungEngine.Utils.ValueCategory.None)
        ...
        /// <param name="category">The value type: none means just a number, distance will format in cm/m/km, angle with an appended degree sign.</param>
        /// <returns>The formatted string.</returns>
        public static string FormatFloat(double value, BalmungEngine.Utils.ValueCategory category)
        {
