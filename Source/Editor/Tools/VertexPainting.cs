    ...
    sealed class VertexPaintingGizmo : GizmoBase
    {
        ...
        public override void Draw(ref RenderContext renderContext)
        {
            ...
            if (_hasHit)
            {
                ...
                if (!_brushModel)
                {
                    _brushModel = BalmungEngine.Content.LoadAsyncInternal<Model>("Editor/Primitives/Sphere");
                }
                if (!_brushMaterial)
                {
                    var material = BalmungEngine.Content.LoadAsyncInternal<Material>(EditorAssets.FoliageBrushMaterial);
                ...
                if (meshDatas != null && _gizmoMode.PreviewVertexSize > Mathf.Epsilon)
                {
                    ...
                    {
                        _verticesPreviewMaterial = BalmungEngine.Content.LoadAsyncInternal<MaterialBase>(EditorAssets.WiresDebugMaterial);
                    }
                    var instanceTransform = _selectedModel.Transform;
            ...
            if (!_vertexColorsPreviewMaterial)
                _vertexColorsPreviewMaterial = BalmungEngine.Content.LoadAsyncInternal<MaterialBase>(EditorAssets.VertexColorsPreviewMaterial).CreateVirtualInstance();
            if (!_vertexColorsPreviewMaterial)
                return;
