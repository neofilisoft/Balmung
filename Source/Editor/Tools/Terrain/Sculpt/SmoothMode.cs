            ...
            for (int i = 0; i < affectedPatches.Count; i++)
            {
                ...
                for (int z = 0; z < patch.ModifiedSize.Y; z++)
                {
                    for (int x = 0; x < patch.ModifiedSize.X; x++)
                    {
                        ...
                        var samplePositionLocal = patch.PatchPositionLocal + new Vector3(localCoordX * BalmungEngine.Terrain.UnitsPerVertex, coordHeight, localCoordY * BalmungEngine.Terrain.UnitsPerVertex);
