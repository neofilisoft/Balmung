...
namespace BalmungEditor.Tools.Terrain.Paint
{
    ...
    /// </summary>
    /// <seealso cref="BalmungEditor.Tools.Terrain.Paint.Mode" />
    [HideInEditor]
    public sealed class SingleLayerMode : Mode
    {
        ...
        /// </summary>
        [EditorOrder(10), Tooltip("The layer to paint on. Terrain material can access a per-layer blend weight to perform material or texture blending."), CustomEditorAlias("BalmungEditor.CustomEditors.Editors.TerrainLayerEditor")]
        ...
        public override unsafe void Apply(ref ApplyParams p)
        {
            ...
            for (int z = 0; z < p.ModifiedSize.Y; z++)
            {
                ...
                for (int x = 0; x < p.ModifiedSize.X; x++)
                {
                    ...
                    var samplePositionLocal = p.PatchPositionLocal + new Vector3(xx * BalmungEngine.Terrain.UnitsPerVertex, 0, zz * BalmungEngine.Terrain.UnitsPerVertex);
