...
using FlaxEngine;
using Object = BalmungEngine.Object;

namespace BalmungEditor.Tools.Terrain
{
    /// <summary>
    /// Gizmo for picking terrain chunks and patches. Managed by the <see cref="EditTerrainGizmoMode"/>.
    /// </summary>
    /// <seealso cref="BalmungEditor.Gizmo.GizmoBase" />
    [HideInEditor]
    public sealed class EditTerrainGizmo : GizmoBase
    {
        ...
        private void GetAssets()
        {
            ...
            _planeModel = BalmungEngine.Content.LoadAsyncInternal<Model>("Editor/Primitives/Plane");
            _highlightTerrainMaterial = BalmungEngine.Content.LoadAsyncInternal<MaterialBase>(EditorAssets.HighlightTerrainMaterial);
            _highlightMaterial = EditorAssets.Cache.HighlightMaterialInstance;
        }

        private BalmungEngine.Terrain SelectedTerrain
        {
            get
            {
                var sceneEditing = Editor.Instance.SceneEditing;
                var terrainNode = sceneEditing.SelectionCount == 1 ? sceneEditing.Selection[0] as TerrainNode : null;
                return (BalmungEngine.Terrain)terrainNode?.Actor;
            }
        }
        ...
        public override void Draw(ref RenderContext renderContext)
        {
            ...
            case EditTerrainGizmoMode.Modes.Add:
            {
                ...
                if (!terrain.HasPatch(ref patchCoord) && _planeModel)
                {
                    ...
                    var patchSize = terrain.ChunkSize * BalmungEngine.Terrain.UnitsPerVertex * BalmungEngine.Terrain.PatchEdgeChunksCount;
        ...
        public override void Update(float dt)
        {
            ...
            if (Mode.EditMode == EditTerrainGizmoMode.Modes.Add)
            {
                ...
                if (terrainNode != null)
                {
                    ...
                    var terrain = (BalmungEngine.Terrain)terrainNode.Actor;
        ...
        private class AddPatchAction : IUndoAction
        {
            ...
            public AddPatchAction(BalmungEngine.Terrain terrain, ref Int2 patchCoord)
            {
                if (terrain == null)
            ...
            public void Do()
            {
                var terrain = Object.Find<BalmungEngine.Terrain>(ref _terrainId);
                if (terrain == null)
                {
            ...
            private void OnPatchEdit(BalmungEngine.Terrain terrain, ref BoundingBox patchBounds)
            {
            ...
            else
            {
                ...
                if (hit != null)
                {
                    if (Mode.EditMode != EditTerrainGizmoMode.Modes.Add)
                    {
                        ...
                        var terrain = (BalmungEngine.Terrain)hit.Actor;
