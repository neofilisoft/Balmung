...
using BalmungEditor.Tools.Terrain.Sculpt;
using BalmungEditor.Tools.Terrain.Undo;
...
namespace BalmungEditor.Tools.Terrain
{
    ...
    public class SculptTerrainGizmoMode : EditorGizmoMode
    {
        ...
        /// </summary>
        public BalmungEngine.Terrain SelectedTerrain
        {
            get
            {
        ...
        public void SetCursor(ref Vector3 hitPosition)
        {
            ...
            for (int patchIndex = 0; patchIndex < patchesCount; patchIndex++)
            {
                ...
                for (int chunkIndex = 0; chunkIndex < BalmungEngine.Terrain.PatchChunksCount; chunkIndex++)
                {
                    terrain.GetChunkBounds(patchIndex, chunkIndex, out tmp);
                    if (!tmp.Intersects(ref brushBounds))
                        continue;

                    var chunkCoord = new Int2(chunkIndex % BalmungEngine.Terrain.PatchEdgeChunksCount, chunkIndex / BalmungEngine.Terrain.PatchEdgeChunksCount);
