...
#include "Engine/Engine/WorkspaceBranding.h"
...
bool Editor::CheckProjectUpgrade()
{
    ...
    if (EditorImpl::IsOldProjectXmlFormat)
    {
        ...
        Project->ProjectPath = root / String::Format(TEXT("{0}.balproj"), codeName);
...
int32 Editor::LoadProduct()
{
    ...
    // Workspace product branding
    WorkspaceBranding::ApplyEditorStartup();
    ...
    if (length > 0)
    {
        ...
        if (projectPath.EndsWith(TEXT(".balproj")))
        {
    ...
    if (CommandLine::Options.NewProject.IsTrue())
    {
        ...
        FileSystem::DirectoryGetFiles(projectFiles, projectPath, TEXT("*.balproj"), DirectorySearchOption::TopDirectoryOnly);
    ...
    if (CommandLine::Options.NewProject.IsTrue())
    {
        ...
        newProject.ProjectPath = projectPath / newProject.Name + TEXT(".balproj");
        newProject.ProjectFolderPath = projectPath;
        newProject.Version = Version(1, 0);
        newProject.Company = Globals::CompanyName;
        ...
        auto& balmungRef = newProject.References.AddOne();
        balmungRef.Name = TEXT("$(EnginePath)/Balmung.balproj");
        balmungRef.Project = nullptr;
    ...
    if (projectPath.IsEmpty())
    {
        ...
        if (FileSystem::ShowOpenFileDialog(
            nullptr,
            StringView::Empty,
            TEXT("Project files (*.balproj)\0*.balproj\0All files (*.*)\0*.*\0"),
            false,
    ...
    else
    {
        ...
        {
            Platform::Fatal(TEXT("Missing project file (*.balproj)."));
            return -2;
        }
        if (projectFiles.Count() > 1)
        {
...
void EditorImpl::OnUpdate()
{
    ...
    WorkspaceBranding::PollEditorChanges();
