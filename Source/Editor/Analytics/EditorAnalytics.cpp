...
void EditorAnalytics::SendEvent(const char* name, Span<Pair<const char*, const char*>> parameters)
{
    ...
    const StringAsANSI<> productName(*Globals::ProductName);
    curl_easy_setopt(curl, CURLOPT_USERAGENT, productName.Get());
...
bool EditorAnalyticsService::Init()
{
    ...
    // If file '%appdata/<CompanyName>/noTracking' exists then do not use analytics (globally)
    String appDataPath;
    ...
    if (FileSystem::FileExists(Globals::StartupFolder / TEXT("noTracking")) ||
        FileSystem::FileExists(appDataPath / Globals::CompanyName / TEXT("noTracking")))
    {
