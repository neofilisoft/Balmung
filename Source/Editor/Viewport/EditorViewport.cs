...
using JsonSerializer = BalmungEngine.Json.JsonSerializer;

namespace BalmungEditor.Viewport
{
    /// <summary>
    /// Editor viewports base class.
    /// </summary>
    /// <seealso cref="BalmungEngine.GUI.RenderOutputControl" />
    public class EditorViewport : RenderOutputControl
    {
        ...
        {
            var viewport = new BalmungEngine.Viewport(0, 0, Width, Height);
        ...
        public override void Update(float deltaTime)
        {
            ...
            if (_isControllingMouse)
            {
                ...
                // Gather input
                {
                    ...
                    _input.IsZooming = wheelInUse && !(_input.IsShiftDown || (!ContainsFocus && BalmungEngine.Input.GetKey(KeyboardKeys.Shift)));
            ...
            else
            {
                ...
                if (ContainsFocus)
                {
                    ...
                    var moveDelta = Vector3.Zero;
                    var mouseDelta = Float2.Zero;
                    if (BalmungEngine.Input.GamepadsCount > 0)
                    {
                        // Gamepads handling
        ...
        {
            var value = BalmungEngine.Input.GetGamepadAxis(InputGamepadIndex.All, axis);
            var deadZone = 0.2f;
