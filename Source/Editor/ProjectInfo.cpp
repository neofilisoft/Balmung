...
bool ProjectInfo::LoadOldProject(const String& projectPath)
{
    ...
    auto& balmungReference = References.AddOne();
    balmungReference.Name = TEXT("$(EnginePath)/Balmung.balproj");
    balmungReference.Project = Load(Globals::StartupFolder / TEXT("Balmung.balproj"));
    if (!balmungReference.Project)
    {
        ShowProjectLoadError(TEXT("Failed to load Balmung Engine project."), projectPath);
        return true;
    }
