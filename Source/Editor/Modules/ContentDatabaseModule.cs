...
namespace BalmungEditor.Modules
{
    ...
    public sealed class ContentDatabaseModule : EditorModule
    {
        ...
        : base(editor)
        {
            ...
            BalmungEngine.Json.JsonSerializer.Settings.Converters.Add(new AssetItemConverter());
        ...
        private static bool RenameAsset(ContentItem el, ref string newPath)
        {
            ...
            if (el.IsAsset)
            {
                ...
                if (BalmungEngine.Content.RenameAsset(oldPath, newPath))
                {
        ...
        public void Delete(ContentItem item, bool deletedByUser = false)
        {
            ...
            if (item is ContentFolder folder)
            {
                ...
                if (deletedByUser && Directory.Exists(path))
                {
                    ...
                    BalmungEngine.Scripting.FlushRemovedObjects();
            ...
            else
            {
                ...
                if (item.IsAsset)
                {
                    ...
                    BalmungEngine.Content.DeleteAsset(path);
                }
            ...
            else
            {
                ...
                for (int i = 0; i < folder.Children.Count; i++)
                {
                    ...
                    else if (canHaveAssets && child is AssetItem childAsset)
                    {
                        ...
                        if (BalmungEngine.Content.GetAssetInfo(child.Path, out var assetInfo))
                        {
        ...
        private void LoadAssets(ContentTreeNode parent, string[] files)
        {
            for (int i = 0; i < files.Length; i++)
            {
                ...
                if (_isDuringFastSetup || !parent.Folder.ContainsChild(path))
                {
                    ...
                    if (BalmungEngine.Content.GetAssetInfo(path, out var assetInfo))
                    {
        ...
        {
            BalmungEngine.Content.AssetDisposing += OnContentAssetDisposing;
            ...
            Proxy.Add(new BalmungLuaScriptProxy());
            Proxy.Add(new BalmungLuaGamePluginProxy());
            Proxy.Add(new Balmung2DCameraRigProxy());
            Proxy.Add(new Balmung2DSortingProxy());
            ...
            Editor.ContentImporting.ImportFileEnd += (obj, failed) =>
            {
                ...
                if (!failed)
                    BalmungEngine.Scripting.InvokeOnUpdate(() => OnImportFileDone(path));
            };
            _enableEvents = true;
        ...
        private void OnImportFileDone(string path)
        {
            ...
            if (item is BinaryAssetItem binaryAssetItem)
            {
                ...
                if (BalmungEngine.Content.GetAssetInfo(binaryAssetItem.Path, out var assetInfo))
                {
