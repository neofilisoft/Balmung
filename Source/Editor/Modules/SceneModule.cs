...
namespace BalmungEditor.Modules
{
    ...
    public sealed class SceneModule : EditorModule
    {
        ...
        /// </summary>
        /// <seealso cref="BalmungEditor.SceneGraph.RootNode" />
        ...
        public void CreateSceneFile(string path)
        {
            ...
            skyLight.CustomTexture = BalmungEngine.Content.LoadAsyncInternal<CubeTexture>(EditorAssets.DefaultSkyCubeTexture);
            skyLight.StaticFlags = StaticFlags.FullyStatic;
            //
            var floor = scene.AddChild<StaticModel>();
            floor.Name = "Floor";
            floor.Scale = new Float3(4, 0.5f, 4);
            floor.Model = BalmungEngine.Content.LoadAsync<Model>(StringUtils.CombinePaths(Globals.EngineContentFolder, "Editor/Primitives/Cube.balmung"));
            if (floor.Model)
            {
                floor.Model.WaitForLoaded();
                floor.SetMaterial(0, BalmungEngine.Content.LoadAsync<MaterialBase>(StringUtils.CombinePaths(Globals.EngineContentFolder, "Engine/WhiteMaterial.balmung")));
            }
