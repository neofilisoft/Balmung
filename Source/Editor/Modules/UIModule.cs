...
using BalmungEditor.GUI.Dialogs;
using BalmungEditor.GUI.Input;
using BalmungEditor.Progress.Handlers;
using BalmungEditor.SceneGraph;
using BalmungEditor.Utilities;
using BalmungEditor.Viewport.Cameras;
using BalmungEditor.Windows;
using FlaxEngine;
using BalmungEngine.GUI;
using BalmungEngine.Json;
using DockHintWindow = BalmungEditor.GUI.Docking.DockHintWindow;
using MasterDockPanel = BalmungEditor.GUI.Docking.MasterDockPanel;
...
namespace BalmungEditor.Modules
{
    ...
    /// </summary>
    /// <seealso cref="BalmungEditor.Modules.EditorModule" />
    public sealed class UIModule : EditorModule
    {
        ...
        {
            InitOrder = -70;
            VisjectSurfaceBackground = BalmungEngine.Content.LoadAsyncInternal<Texture>("Editor/VisjectSurface");
        ...
        public void UpdateStatusBar()
        {
            ...
            var contentStats = BalmungEngine.Content.Stats;
        ...
        public override void OnUpdate()
        {
            ...
            else if (BalmungEngine.Content.Stats.LoadingAssetsCount != _contentStats.LoadingAssetsCount)
            {
                UpdateStatusBar();
            }
        ...
        private void NewProject()
        {
            ...
            if (FileSystem.ShowSaveFileDialog(Editor.Windows.MainWindow, null, "Project files (*.balproj)\0*.balproj\0All files (*.*)\0*.*\0", false, "Create project file", out var files))
                return;
        ...
        private void OpenProject()
        {
            ...
            if (FileSystem.ShowOpenFileDialog(Editor.Windows.MainWindow, null, "Project files (*.balproj)\0*.balproj\0All files (*.*)\0*.*\0", false, "Select project file", out var files))
                return;
            if (files != null && files.Length > 0)
            {
