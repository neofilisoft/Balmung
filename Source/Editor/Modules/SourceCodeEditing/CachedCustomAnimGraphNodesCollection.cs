...
namespace BalmungEditor.Modules.SourceCodeEditing
{
    /// <summary>
    /// Cached collection of custom nodes types including cached node archetypes for each one of them.
    /// </summary>
    /// <seealso cref="BalmungEditor.Modules.SourceCodeEditing.CachedTypesCollection" />
    [HideInEditor]
    public class CachedCustomAnimGraphNodesCollection : CachedTypesCollection
    {
        ...
        private NodeArchetype GetArchetype(MethodBase method)
        {
            ...
            try
            {
                ...
                HasTypeFromGameScripts |= BalmungEngine.Scripting.IsTypeFromGameScripts(method.DeclaringType) || BalmungEngine.Scripting.IsTypeFromGameScripts(type);
