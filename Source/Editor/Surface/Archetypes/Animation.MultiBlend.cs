...
using BalmungEditor.Surface.Undo;
using FlaxEngine;
...
namespace BalmungEditor.Surface.Archetypes
{
    ...
    /// </summary>
    /// <seealso cref="BalmungEngine.GUI.ContainerControl" />
    [HideInEditor]
    public class BlendPointsEditor : ContainerControl
    {
        ...
        private void AddPoint(Float2 location)
        {
            ...
            if (id == Guid.Empty)
            {
                ...
                var ids = BalmungEngine.Content.GetAllAssetsByType(typeof(BalmungEngine.Animation));
                if (ids.Length != 0)
                    id = ids[0];
                else
                    return;
        ...
        public override bool OnMouseUp(Float2 location, MouseButton button)
        {
            ...
            {
                // Show context menu
                var menu = new BalmungEditor.GUI.ContextMenu.ContextMenu();
        ...
        private void OnAddPoint(BalmungEditor.GUI.ContextMenu.ContextMenuButton b)
        {
            AddPoint(BlendPointPosToBlendSpacePos((Float2)b.Tag));
        }

        private void OnRemovePoint(BalmungEditor.GUI.ContextMenu.ContextMenuButton b)
        {
    ...
    public static partial class Animation
    {
        ...
        /// </summary>
        /// <seealso cref="BalmungEditor.Surface.SurfaceNode" />
        public abstract class MultiBlend : SurfaceNode
        {
            ...
            {
                var layoutOffsetY = BalmungEditor.Surface.Constants.LayoutOffsetY;
                ...
                _animationPicker = new AssetPicker(new ScriptType(typeof(BalmungEngine.Animation)), new Float2(_selectedAnimation.Left, _selectedAnimation.Bottom + 4))
                {
                    Parent = this
                };
            ...
            private void OnSelectedAnimationPopupShowing(ComboBox comboBox)
            {
                ...
                for (var i = 0; i < count; i++)
                {
                    ...
                    if (BalmungEngine.Content.GetAssetInfo(animId, out var assetInfo))
            ...
            public virtual void UpdateUI(int selectedIndex, bool isValid, ref Float4 data0, ref Guid data1)
            {
                if (isValid)
                {
                    ...
                    var path = string.Empty;
                    if (BalmungEngine.Content.GetAssetInfo(data1, out var assetInfo))
                        path = Path.GetFileNameWithoutExtension(assetInfo.Path);
            ...
            {
                BalmungEngine.Json.JsonSerializer.ParseID(text, out var id);
                if (id != Guid.Empty)
                {
                    for (int i = 5; i < Values.Length; i += 2)
                    {
        ...
        public class MultiBlend1D : MultiBlend
        {
            ...
            : base(id, context, nodeArch, groupArch)
            {
                ...
                var size = Width - BalmungEditor.Surface.Constants.NodeMarginX * 2.0f;
                _editor = new BlendPointsEditor(this, false, BalmungEditor.Surface.Constants.NodeMarginX, _animationX.Bottom + 4.0f, size, 120.0f);
                _editor.Parent = this;
            }
        ...
        public class MultiBlend2D : MultiBlend
        {
            ...
            : base(id, context, nodeArch, groupArch)
            {
                ...
                var size = Width - BalmungEditor.Surface.Constants.NodeMarginX * 2.0f;
                _editor = new BlendPointsEditor(this, true, BalmungEditor.Surface.Constants.NodeMarginX, _animationY.Bottom + 4.0f, size, size);
                _editor.Parent = this;
            }
            ...
            private void CacheTriangles()
            {
                ...
                // Triangulate
                _triangles = BalmungEngine.Utilities.Delaunay2D.Triangulate(vertices);
                _triangleColors = null;
