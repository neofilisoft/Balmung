...
using BalmungEditor.CustomEditors.Dedicated;
using BalmungEditor.GUI.ContextMenu;
using BalmungEditor.GUI.Drag;
using BalmungEditor.Scripting;
using BalmungEditor.Surface.Elements;
using FlaxEngine;
using BalmungEngine.GUI;
using BalmungEngine.Utilities;

namespace BalmungEditor.Surface.Archetypes
{
    /// <summary>
    ...
    public static class BehaviorTree
    {
        ...
        internal class NodeBase : SurfaceNode
        {
            ...
            public override void OnLoaded(SurfaceNodeActions action)
            {
                ...
                if (_type != null)
                {
                    ...
                    try
                    {
                        ...
                        BalmungEngine.Json.JsonSerializer.LoadFromBytes(Instance, instanceData, Globals.EngineBuildNumber);
                    }
                    catch (Exception ex)
                    {
                    ...
                    try
                    {
                        if (Instance != null)
                        {
                            ...
                            if (instanceData == null || instanceData.Length == 0)
                            {
                                ...
                                instanceData = BalmungEngine.Json.JsonSerializer.SaveToBytes(defaultInstance);
                            }
                            BalmungEngine.Json.JsonSerializer.LoadFromBytes(Instance, instanceData, Globals.EngineBuildNumber);
                        }
                    }
            ...
            public override void OnDestroy()
            {
                ...
                _debugInfo = null;
                _type = ScriptType.Null;
                BalmungEngine.Object.Destroy(ref Instance);
        ...
        internal class Node : NodeBase
        {
            ...
            public override void OnShowSecondaryContextMenu(BalmungEditor.GUI.ContextMenu.ContextMenu menu, Float2 location)
            {
            ...
            public override void OnLoaded(SurfaceNodeActions action)
            {
                ...
                _input.ConnectionOffset = new Float2(0, BalmungEditor.Surface.Constants.BoxSize * -0.5f);
                _output.ConnectionOffset = new Float2(0, BalmungEditor.Surface.Constants.BoxSize * 0.5f);
            ...
            public override void ResizeAuto()
            {
                ...
                foreach (var decorator in decorators)
                {
                    ...
                    width = Mathf.Max(width, decorator.Width - BalmungEditor.Surface.Constants.NodeCloseButtonSize - 2 * DecoratorsMarginX);
                }
                Size = new Float2(width + BalmungEditor.Surface.Constants.NodeMarginX * 2 + BalmungEditor.Surface.Constants.NodeCloseButtonSize, height + BalmungEditor.Surface.Constants.NodeHeaderSize + BalmungEditor.Surface.Constants.NodeFooterSize);
                UpdateRectangles();
            }
            ...
            protected override void UpdateRectangles()
            {
                ...
                const float footerSize = BalmungEditor.Surface.Constants.NodeFooterSize;
                const float headerSize = BalmungEditor.Surface.Constants.NodeHeaderSize;
                const float closeButtonMargin = BalmungEditor.Surface.Constants.NodeCloseButtonMargin;
                const float closeButtonSize = BalmungEditor.Surface.Constants.NodeCloseButtonSize;
        ...
        internal class Decorator : NodeBase
        {
            ...
            protected override Float2 CalculateNodeSize(float width, float height)
            {
                ...
                return new Float2(width + BalmungEditor.Surface.Constants.NodeCloseButtonSize * 2 + DecoratorsMarginX * 2, height + BalmungEditor.Surface.Constants.NodeHeaderSize);
            }
