...
namespace BalmungEditor.Surface
{
    ...
    {
        /// <inheritdoc />
        public MaterialSurface(IVisjectSurfaceOwner owner, Action onSave = null, BalmungEditor.Undo undo = null)
        ...
        protected override void HandleDragDropAssets(List<AssetItem> objects, DragDropEventArgs args)
        {
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                if (assetItem.IsOfType<Texture>())
                {
                    ...
                    var obj = BalmungEngine.Content.LoadAsync<Texture>(assetItem.ID);
                    if (obj)
                    {
                        Thread.Sleep(50);
