...
using Animation = BalmungEditor.Surface.Archetypes.Animation;
...
namespace BalmungEditor.Surface
{
    ...
    public class AnimGraphSurface : VisjectSurface
    {
        ...
        /// <inheritdoc />
        public AnimGraphSurface(IVisjectSurfaceOwner owner, Action onSave, BalmungEditor.Undo undo)
        ...
        private void OnScriptsReloadBegin()
        {
            ...
            if (!hasTypeFromGameScripts && RootContext != null)
            {
                ...
                {
                    if (BalmungEngine.Scripting.IsTypeFromGameScripts(param.Type.Type))
                    {
        ...
        protected override bool ValidateDragItem(AssetItem assetItem)
        {
            if (assetItem.IsOfType<SkeletonMask>())
                return true;
            if (assetItem.IsOfType<BalmungEngine.Animation>())
                return true;
