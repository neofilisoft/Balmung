    ...
    for (int32 i = 0; i < msgData.ItemsCount; i++)
    {
        ...
        if (sceneObject)
        {
            ...
            else if (msgDataItem.ParentId.IsValid())
            {
                ...
                AssetInfo assetInfo;
                if (!Content::GetAssetInfo(msgDataItem.ParentId, assetInfo) || assetInfo.TypeName != TEXT("BalmungEngine.SceneAsset"))
                {
