...
namespace LevelImpl
{
    ...
    SceneResult loadScene(SceneLoader& loader, rapidjson_balmung::Document& document, Scene** outScene = nullptr, float* timeBudget = nullptr);
    SceneResult loadScene(SceneLoader& loader, rapidjson_balmung::Value& data, int32 engineBuild, Scene** outScene = nullptr, StringView assetPath = StringView(), float* timeBudget = nullptr);
    bool unloadScene(Scene* scene);
    bool unloadScenes();
    bool saveScene(Scene* scene);
    bool saveScene(Scene* scene, const String& path);
    bool saveScene(Scene* scene, rapidjson_balmung::StringBuffer& outBuffer, bool prettyJson);
    bool saveScene(Scene* scene, rapidjson_balmung::StringBuffer& outBuffer, JsonWriter& writer);
...
public:
    ...
    SceneResult Do(Context& context) override
    {
        ...
        struct SceneData
        {
            Guid ID;
            String Name;
            rapidjson_balmung::StringBuffer Data;
...
SceneResult LevelImpl::loadScene(SceneLoader& loader, rapidjson_balmung::Document& document, Scene** outScene, float* timeBudget)
{
...
SceneResult LevelImpl::loadScene(SceneLoader& loader, rapidjson_balmung::Value& data, int32 engineBuild, Scene** outScene, StringView assetPath, float* timeBudget)
{
...
bool LevelImpl::saveScene(Scene* scene, rapidjson_balmung::StringBuffer& outBuffer, bool prettyJson)
{
    PROFILE_CPU_NAMED("Level.SaveScene");
    PROFILE_MEM(Level);
    if (prettyJson)
...
bool LevelImpl::saveScene(Scene* scene, rapidjson_balmung::StringBuffer& outBuffer, JsonWriter& writer)
{
    ASSERT(scene);
    const auto sceneId = scene->GetID();
    ...
    writer.StartObject();
    {
        ...
        writer.String("BalmungEngine.SceneAsset");
...
bool Level::SaveSceneToBytes(Scene* scene, rapidjson_balmung::StringBuffer& outData, bool prettyJson)
{
    ASSERT(scene);
    ScopeLock lock(_sceneActionsLocker);
    Stopwatch stopwatch;
...
{
    Array<byte> data;
    rapidjson_balmung::StringBuffer sceneData;
    if (!SaveSceneToBytes(scene, sceneData, prettyJson))
    {
        data.Set((const byte*)sceneData.GetString(), (int32)sceneData.GetSize() * sizeof(rapidjson_balmung::StringBuffer::Ch));
    }
    return data;
}
