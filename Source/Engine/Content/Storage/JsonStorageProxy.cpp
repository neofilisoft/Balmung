...
void FindObjectIds(const rapidjson_balmung::Value& obj, const rapidjson_balmung::Document& document, HashSet<Guid>& ids, const char* parentName = nullptr)
{
    if (obj.IsObject())
    {
        for (rapidjson_balmung::Value::ConstMemberIterator i = obj.MemberBegin(); i != obj.MemberEnd(); ++i)
        {
