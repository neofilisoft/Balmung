...
public sealed class VulkanSdk : Sdk
{
    ...
    public VulkanSdk()
    {
        var platform = Balmung.Build.Platform.BuildTargetPlatform;
        ...
        if (vulkanSdk == null || !Directory.Exists(vulkanSdk))
        {
            if (platform == TargetPlatform.Mac)
            {
                ...
                if (Directory.Exists(path))
                {
                    ...
                    if (subDirs.Length != 0)
                    {
                        Balmung.Build.Utilities.SortVersionDirectories(subDirs);
    ...
    public bool TryGetIncludePath(TargetPlatform platform, out string includesFolderPath)
    {
        ...
        if (IsValid)
        {
            ...
            {
                if (platform != Balmung.Build.Platform.BuildTargetPlatform)
                {
