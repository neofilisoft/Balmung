...
bool ImportModel::TryGetImportOptions(const StringView& path, Options& options)
{
    if (FileSystem::FileExists(path))
    {
        ...
        {
            // Check import meta
            rapidjson_balmung::Document metadata;
...
void CloneObject(rapidjson_balmung::StringBuffer& buffer, SceneObject* src, SceneObject* dst, bool stripName = false)
{
    // Serialize source
    buffer.Clear();
...
CreateAssetResult ImportModel::Import(CreateAssetContext& context)
{
    ...
    if (options.Type == ModelTool::ModelType::Prefab)
    {
        ...
        Function<bool(Options& splitOptions, const StringView& objectName, String& outputPath, MeshData* meshData)> splitImport = [&context, &autoImportOutput, &objectNames](Options& splitOptions, const StringView& objectName, String& outputPath, MeshData* meshData)
        {
            ...
            outputPath = autoImportOutput / String(StringUtils::GetFileNameWithoutExtension(context.TargetAssetPath)) + TEXT(" ") + postFix + TEXT(".balmung");
    ...
    else if (options.SplitObjects)
    {
        ...
        splitImport.Bind([&context](Options& splitOptions, const StringView& objectName)
        {
            ...
            if (splitPos != -1)
                postFix = postFix.Substring(splitPos + 1);
            const String outputPath = String(StringUtils::GetPathWithoutExtension(context.TargetAssetPath)) + TEXT(" ") + postFix + TEXT(".balmung");
            return AssetsImportingManager::Import(context.InputPath, outputPath, &splitOptions);
        });
        auto splitOptions = options;
        switch (options.Type)
    ...
    rapidjson_balmung::StringBuffer importOptionsMetaBuffer;
...
CreateAssetResult ImportModel::CreatePrefab(CreateAssetContext& context, const ModelData& data, const Options& options, const Array<PrefabObject>& prefabObjects)
{
    ...
    rapidjson_balmung::StringBuffer jsonBuffer;
