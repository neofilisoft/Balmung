...
void Log::Logger::ProcessLogMessage(LogType type, const StringView& msg, fmt_balmung::memory_buffer& w)
{
    const TimeSpan time = DateTime::Now() - LogStartTime;
    fmt_balmung::format(w, TEXT("[ {0} ]: [{1}] "), *time.ToString('a'), ToString(type));
    ...
    if (hasWindowsNewLine)
    {
        ...
        //fmt_balmung::format(w, TEXT("{}"), (const Char*)msgStream.Get());
        return;
    }
#endif
