...
namespace
{
    const char* InBuiltTypesTypeNames[40] =
    {
        ...
        "",// Structure
        "BalmungEngine.Asset",// Asset
        "System.Byte[]",// Blob
        "",// Enum
        "BalmungEngine.Float2",// Float2
        "BalmungEngine.Float3",// Float3
        "BalmungEngine.Float4",// Float4
        "BalmungEngine.Color",// Color
        "System.Guid",// Guid
        "BalmungEngine.BoundingBox",// BoundingBox
        "BalmungEngine.BoundingSphere",// BoundingSphere
        "BalmungEngine.Quaternion",// Quaternion
        "BalmungEngine.Transform",// Transform
        "BalmungEngine.Rectangle",// Rectangle
        "BalmungEngine.Ray",// Ray
        "BalmungEngine.Matrix",// Matrix
        "System.Object[]",// Array
        ...
        "BalmungEngine.Int2",// Int2
        "BalmungEngine.Int3",// Int3
        "BalmungEngine.Int4",// Int4
        "System.Int16",// Int16
        "System.UInt16",// Uint16
        "BalmungEngine.Double2",// Double2
        "BalmungEngine.Double3",// Double3
        "BalmungEngine.Double4",// Double4
        // @formatter:on
    };
}
...
VariantType::VariantType(const StringAnsiView& typeName)
{
    ...
    {
        // Aliases
        if (typeName == "BalmungEngine.Vector2")
        {
            new(this) VariantType(Vector2);
            return;
        }
        if (typeName == "BalmungEngine.Vector3")
        {
            new(this) VariantType(Vector3);
            return;
        }
        if (typeName == "BalmungEngine.Vector4")
        {
...
void Variant::Inline()
{
    ...
    if (Type.Type == VariantType::Structure && AsBlob.Data && AsBlob.Length <= sizeof(Matrix))
    {
        ...
        {
            // Aliases
            if (StringUtils::Compare(Type.TypeName, "BalmungEngine.Vector2") == 0)
                type = VariantType::Types::Vector2;
            else if (StringUtils::Compare(Type.TypeName, "BalmungEngine.Vector3") == 0)
                type = VariantType::Types::Vector3;
            else if (StringUtils::Compare(Type.TypeName, "BalmungEngine.Vector4") == 0)
...
Variant Variant::Enum(const VariantType& type, const uint64 value)
{
    Variant v;
    v.SetType(type);
    v.AsEnum = value;
