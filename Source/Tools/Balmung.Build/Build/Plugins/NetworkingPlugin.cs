...
using Balmung.Build.Bindings;
using Mono.Cecil;
using Mono.Cecil.Cil;

namespace Balmung.Build.Plugins
{
    /// <summary>
    /// Balmung.Build plugin for Networking extensions support. Generates required bindings glue code for automatic types replication and RPCs invoking. 
    /// </summary>
    /// <seealso cref="Balmung.Build.Plugin" />
    internal sealed class NetworkingPlugin : Plugin
    {
        ...
        internal const string NetworkReplicatedAttribute = "BalmungEngine.NetworkReplicatedAttribute";
        ...
        private static readonly Dictionary<string, InBuildSerializer> _inBuildSerializers = new Dictionary<string, InBuildSerializer>()
        {
            ...
            { "BalmungEngine.Vector2", new InBuildSerializer("WriteVector2", "ReadVector2") },
            { "BalmungEngine.Vector3", new InBuildSerializer("WriteVector3", "ReadVector3") },
            { "BalmungEngine.Vector4", new InBuildSerializer("WriteVector4", "ReadVector4") },
            { "BalmungEngine.Float2", new InBuildSerializer("WriteFloat2", "ReadFloat2") },
            { "BalmungEngine.Float3", new InBuildSerializer("WriteFloat3", "ReadFloat3") },
            { "BalmungEngine.Float4", new InBuildSerializer("WriteFloat4", "ReadFloat4") },
            { "BalmungEngine.Quaternion", new InBuildSerializer("WriteQuaternion", "ReadQuaternion") },
            { "BalmungEngine.Ray", new InBuildSerializer("WriteRay", "ReadRay") },
        };
        ...
        private void OnGenerateCSharpTypeSerialize(Builder.BuildData buildData, ApiTypeInfo typeInfo, StringBuilder contents, string indent, bool serialize)
        {
            var mode = serialize ? "true" : "false";
            contents.AppendLine();
            contents.Append(indent).Append("public void ").Append(serialize ? Thunk1 : Thunk2).AppendLine("(BalmungEngine.Networking.NetworkStream stream)");
            contents.Append(indent).AppendLine("{");
            if (typeInfo is ClassInfo)
            {
                contents.Append(indent).AppendLine($"    BalmungEngine.Networking.NetworkReplicator.InvokeSerializer(typeof({typeInfo.Name}), this, stream, {mode});");
            }
            else
            {
                if (typeInfo.IsPod)
                {
                    ...
                    contents.Append(indent).AppendLine($"        BalmungEngine.Networking.NetworkReplicator.InvokeSerializer(typeof({typeInfo.Name}), new IntPtr(ptr), stream, {mode});");
                }
                else
                {
        ...
        private void OnPatchDotNetAssembly(Builder.BuildData buildData, NativeCpp.BuildOptions buildOptions, Task buildTask, string assemblyPath)
        {
            ...
            using (AssemblyDefinition assembly = AssemblyDefinition.ReadAssembly(assemblyPath, new ReaderParameters { ReadWrite = true, ReadSymbols = true, AssemblyResolver = assemblyResolver }))
            {
                ...
                module.GetType("BalmungEngine.Networking.NetworkStream", out context.NetworkStreamType);
                ...
                if (context.AddSerializers.Count != 0 || context.MethodRPCs.Count != 0)
                {
                    ...
                    module.GetType("BalmungEngine.ModuleInitializerAttribute", out var moduleInitializerAttribute);
                    ...
                    module.GetType("BalmungEngine.Networking.NetworkReplicator", out var networkReplicatorType);
        ...
        private static void GenerateTypeNetworking(ref DotnetContext context, TypeDefinition type)
        {
            ...
            if (type.IsClass)
            {
                ...
                for (int i = 0; i < methodsCount; i++)
                {
                    ...
                    var attribute = method.CustomAttributes.FirstOrDefault(x => x.AttributeType.FullName == "BalmungEngine.NetworkRpcAttribute");
                    if (attribute != null)
                    {
        ...
        private static void GenerateTypeSerialization(ref DotnetContext context, TypeDefinition type)
        {
            ...
            // Skip native types
            var isNative = type.HasAttribute("BalmungEngine.UnmanagedAttribute");
            if (isNative)
                return;
            ...
            var isINetworkSerializable = type.HasInterface("BalmungEngine.Networking.INetworkSerializable");
            ...
            if (isINetworkSerializable)
            {
                ...
                {
                    if (m.HasBody && m.Parameters.Count == 1 && m.Parameters[0].ParameterType.FullName == "BalmungEngine.Networking.NetworkStream")
                    {
                        if (m.Name == "Serialize")
        ...
        private static MethodDefinition GenerateDotnetSerializer(ref DotnetContext context, TypeDefinition type, bool serialize, string name)
        {
            ...
            // Serialize base type
            if (type.BaseType != null && type.BaseType.FullName != "System.ValueType" && type.BaseType.FullName != "BalmungEngine.Object" && type.BaseType.CanBeResolved())
            {
        ...
        private static MethodDefinition GenerateNativeSerializer(ref DotnetContext context, TypeDefinition type, bool serialize, string name)
        {
            ...
            module.GetType("BalmungEngine.Object", out var scriptingObjectType);
            ...
            // <type> instance = (<type>)BalmungEngine.Object.FromUnmanagedPtr(instancePtr)
            ...
            // NetworkStream stream = (NetworkStream)BalmungEngine.Object.FromUnmanagedPtr(streamPtr)
        ...
        private static void GenerateDotNetRPCBody(ref DotnetContext context, TypeDefinition type, MethodDefinition method, CustomAttribute attribute, TypeReference networkStreamType)
        {
            ...
            // Generate static method to execute RPC locally
            {
                ...
                for (int i = 0; i < method.Parameters.Count; i++)
                {
                    ...
                    // Special handling of Rpc Params
                    if (string.Equals(parameterType.FullName, "BalmungEngine.Networking.NetworkRpcParams", StringComparison.OrdinalIgnoreCase))
                    {
            ...
            // Inject custom code before RPC method body to invoke it
            {
                ...
                module.GetType("BalmungEngine.Networking.NetworkManagerMode", out var networkManagerModeType);
                module.GetType("BalmungEngine.Networking.NetworkManager", out var networkManagerType);
