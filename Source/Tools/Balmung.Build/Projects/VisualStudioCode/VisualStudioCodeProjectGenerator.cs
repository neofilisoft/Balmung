...
namespace Balmung.Build.Projects.VisualStudioCode
{
    /// <summary>
                ...
                json.BeginRootObject();
                {
                                ...
                                json.BeginObject();
                                {
                                    ...
                                    if (outputType != TargetOutputType.Executable && configuration.Name.StartsWith("Editor."))
                                    {
                                        if (configuration.Platform == TargetPlatform.Windows)
                                        {
                                            ...
                                            json.AddField("program", Path.Combine(Globals.EngineRoot, "Binaries", "Editor", editorFolder, configuration.ConfigurationName, "BalmungEditor.exe"));
                                        }
                                        else if (configuration.Platform == TargetPlatform.Linux)
                                            json.AddField("program", Path.Combine(Globals.EngineRoot, "Binaries", "Editor", "Linux", configuration.ConfigurationName, "BalmungEditor"));
                                        else if (configuration.Platform == TargetPlatform.Mac)
                                            json.AddField("program", Path.Combine(Globals.EngineRoot, "Binaries", "Editor", "Mac", configuration.ConfigurationName, "BalmungEditor"));
                                    ...
                                    json.AddField("visualizerFile", Path.Combine(Globals.EngineRoot, "Source", "balmung.natvis"));
                                }
                                json.EndObject();
                            }
                        }
                            ...
                            foreach (var configuration in csharpProject.Configurations)
                            {
                                ...
                                json.BeginObject();
                                {
                                    ...
                                    if (configuration.Platform == Platform.BuildPlatform.Target)
                                    {
                                        ...
                                        json.AddField("program", Path.Combine(Globals.EngineRoot, "Binaries", "Editor", editorFolder, configuration.ConfigurationName, "BalmungEditor"));
                                        json.BeginArray("args");
                                        {
                                            if (configuration.Platform == TargetPlatform.Linux || configuration.Platform == TargetPlatform.Mac)
                                                json.AddUnnamedField("-std");
                    ...
                    if (hasDotnetProjects)
                    {
                        json.BeginObject();
                        {
                            ...
                            json.AddField("request", "attach");
                            json.AddField("processName", "BalmungEditor");
                        }
                        json.EndObject();
                    }
                    if (hasMonoProjects)
