...
namespace Balmung.Build.Projects.VisualStudio
{
    ...
    /// </summary>
    /// <seealso cref="Balmung.Build.Projects.VisualStudio.VisualStudioProjectGenerator" />
    public class CSSDKProjectGenerator : VisualStudioProjectGenerator
    {
        ...
        public override void GenerateProject(Project project, string solutionPath, bool isMainProject)
        {
            ...
            // Custom .targets file for overriding MSBuild build tasks, only invoke Balmung.Build once per Flax project           
            var balmungBuildTargetsFilename = isMainProject ? "Balmung.Build.CSharp.targets" : "Balmung.Build.CSharp.SkipBuild.targets";
            var cacheProjectsPath = Utilities.MakePathRelativeTo(Path.Combine(Globals.Root, "Cache", "Projects"), projectDirectory);
            var balmungBuildTargetsPath = !string.IsNullOrEmpty(cacheProjectsPath) ? Path.Combine(cacheProjectsPath, balmungBuildTargetsFilename) : balmungBuildTargetsFilename;
            csProjectFileContent.AppendLine(string.Format("    <CustomAfterMicrosoftCommonTargets>$(MSBuildThisFileDirectory){0}</CustomAfterMicrosoftCommonTargets>", balmungBuildTargetsPath));
