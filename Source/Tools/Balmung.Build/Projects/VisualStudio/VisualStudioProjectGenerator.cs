...
namespace Balmung.Build.Projects.VisualStudio
{
    /// <summary>
        ...
        private void GenerateAsciiSolution(Solution solution)
        {
            ...
            // Global configuration
            {
                ...
                foreach (var project in projects)
                {
                    ...
                    if (project.Name == "BuildScripts" || project.Name == "Balmung.Build" || project.Name == "Balmung.Build.Tests")
                        continue;
            ...
            if (Version >= VisualStudioVersion.VisualStudio2022)
            {
                ...
                var editorPath = Utilities.NormalizePath(Path.Combine(Globals.EngineRoot, Platform.GetEditorBinaryDirectory(), configuration, $"BalmungEditor{Utilities.GetPlatformExecutableExt()}")).Replace('\\', '/');
            ...
            // Custom MSBuild .targets file to prevent building Flax C#-projects directly with MSBuild
            {
                ...
                targetsFileContent.AppendLine("  <!-- Prevent building projects with MSBuild, let Balmung.Build handle the building process -->");
                targetsFileContent.AppendLine("  <Target Name=\"Build\" Condition=\"'false' == 'true'\" />");
                targetsFileContent.AppendLine("</Project>");

                Utilities.WriteFileIfChanged(Path.Combine(Globals.Root, "Cache", "Projects", "Balmung.Build.CSharp.SkipBuild.targets"), targetsFileContent.ToString());
            }

            // Override MSBuild build tasks to run Balmung.Build in C#-only projects
            {
                ...
                var targetsFileContent = new StringBuilder();
                targetsFileContent.AppendLine("<Project>");
                targetsFileContent.AppendLine("  <!-- Custom Balmung.Build scripts for C# projects. -->");
                ...
                Utilities.WriteFileIfChanged(Path.Combine(Globals.Root, "Cache", "Projects", "Balmung.Build.CSharp.targets"), targetsFileContent.ToString());
