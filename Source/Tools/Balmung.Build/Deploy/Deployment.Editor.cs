        ...
        public class Editor
        {
            ...
            public static void Package()
            {
                    ...
                    DeployEditorBinaries(configuration);
                {
                    ...
                    var buildToolExe = Platform.BuildTargetPlatform == TargetPlatform.Windows ? "Balmung.Build.exe" : "Balmung.Build";
                    if (!File.Exists(Path.Combine(src, buildToolExe)))
                        buildToolExe = Platform.BuildTargetPlatform == TargetPlatform.Windows ? "Balmung.Build.exe" : "Balmung.Build";
                    DeployFile(src, dst, buildToolExe);
                    CodeSign(Path.Combine(dst, buildToolExe));
                    var buildToolDll = "Balmung.Build.dll";
                    if (!File.Exists(Path.Combine(src, buildToolDll)))
                        buildToolDll = "Balmung.Build.dll";
                    DeployFile(src, dst, buildToolDll);
                    CodeSign(Path.Combine(dst, buildToolDll));
                    var buildToolXml = "Balmung.Build.xml";
                    if (!File.Exists(Path.Combine(src, buildToolXml)))
                        buildToolXml = "Balmung.Build.xml";
                    DeployFile(src, dst, buildToolXml, true);
                    var buildToolPdb = "Balmung.Build.pdb";
                    if (!File.Exists(Path.Combine(src, buildToolPdb)))
                        buildToolPdb = "Balmung.Build.pdb";
                    DeployFile(src, dst, buildToolPdb);
                    var buildToolDeps = "Balmung.Build.deps.json";
                    if (!File.Exists(Path.Combine(src, buildToolDeps)))
                        buildToolDeps = "Balmung.Build.deps.json";
                    DeployFile(src, dst, buildToolDeps);
                    var buildToolRuntimeConfig = "Balmung.Build.runtimeconfig.json";
                    if (!File.Exists(Path.Combine(src, buildToolRuntimeConfig)))
                        buildToolRuntimeConfig = "Balmung.Build.runtimeconfig.json";
                    DeployFile(src, dst, buildToolRuntimeConfig);
                ...
                // Deploy sources
                {
                    ...
                    DeployFile(RootPath, OutputPath, "Source", "balmung.natvis");
                    DeployFile(RootPath, OutputPath, "Source", "BalmungEditor.Build.cs");
                    DeployFile(RootPath, OutputPath, "Source", "BalmungEngine.Gen.h");
                    DeployFile(RootPath, OutputPath, "Source", "BalmungGame.Build.cs");

                    // Mark deployed sources as already prebuilt
                    Utilities.ReplaceInFile(Path.Combine(OutputPath, "Source/BalmungEditor.Build.cs"), "IsPreBuilt = false;", "IsPreBuilt = true;");
                    Utilities.ReplaceInFile(Path.Combine(OutputPath, "Source/BalmungGame.Build.cs"), "IsPreBuilt = false;", "IsPreBuilt = true;");
                }

                // Deploy project
                DeployFile(RootPath, OutputPath, "Balmung.balproj");
                ...
                if (Platform.BuildTargetPlatform == TargetPlatform.Mac)
                {
                    ...
                    var appPath = Path.Combine(Deployer.PackageOutputPath, "BalmungEditor.app");
                    ...
                    var balmungProject = EngineTarget.EngineProject;
                    plist = plist.Replace("{Version}", balmungProject.Version.ToString());
                    plist = plist.Replace("{Copyright}", balmungProject.Copyright);
                    plist = plist.Replace("{Executable}", "BalmungEditor");
                    ...
                    // Build a disk image
                    var dmgPath = Path.Combine(Deployer.PackageOutputPath, "BalmungEditor.dmg");
                    Log.Info(string.Empty);
                ...
                if (unix)
                {
                    ...
                    editorPackageZipPath = Path.Combine(Deployer.PackageOutputPath, $"BalmungEditor{Enum.GetName(typeof(TargetPlatform), Platform.BuildTargetPlatform)}.zip");
            ...
            private static void DeployEditorBinaries(TargetConfiguration configuration)
            {
                ...
                DeployFile(src, dst, "BalmungEditor.Build.json");
                DeployFile(src, dst, "BalmungEngine.CSharp.runtimeconfig.json");
                DeployFile(src, dst, "BalmungEngine.CSharp.pdb");
                DeployFile(src, dst, "BalmungEngine.CSharp.xml");
                ...
                if (Platform.BuildTargetPlatform == TargetPlatform.Windows)
                {
                    ...
                    var editorExeName = "BalmungEditor.exe";
                    ...
                    DeployFile(src, dst, "BalmungEditor.lib");
                    DeployFiles(src, dst, "*.dll");
                    CodeSign(Path.Combine(dst, "BalmungEngine.CSharp.dll"));

                    // Deploy debug symbols files
                    DeployFiles(src, dstDebug, "*.pdb");
                    File.Delete(Path.Combine(dstDebug, "BalmungEngine.CSharp.pdb"));
                    ...
                    if (configuration == TargetConfiguration.Development)
                    {
                        ...
                        DeployFile(src, dst, "BalmungEditor.pdb");
                    }
                }
                else if (Platform.BuildTargetPlatform == TargetPlatform.Linux)
                {
                    // Deploy binaries
                    DeployFile(src, dst, "BalmungEditor");
                    DeployFiles(src, dst, "*.dll");
                    DeployFiles(src, dst, "*.so");
                    DeployFile(src, dst, "Logo.png");

                    // Optimize package size
                    Utilities.Run("strip", "BalmungEditor", null, dst, Utilities.RunOptions.None);
                    Utilities.Run("strip", "libBalmungEditor.so", null, dst, Utilities.RunOptions.None);
                }
                else if (Platform.BuildTargetPlatform == TargetPlatform.Mac)
                {
                    ...
                    // Optimize package size
                    Utilities.Run("strip", "BalmungEditor", null, dst, Utilities.RunOptions.None);
                    Utilities.Run("strip", "BalmungEngine.dylib", null, dst, Utilities.RunOptions.None);
                    Utilities.Run("strip", "libMoltenVK.dylib", null, dst, Utilities.RunOptions.None);

                    // Sign binaries
                    CodeSign(Path.Combine(dst, "BalmungEditor"));
                    CodeSign(Path.Combine(dst, "BalmungEngine.dylib"));
