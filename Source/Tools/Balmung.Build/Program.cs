    ...
    class Program
    {
        static int Main()
        {
            ...
            try
            {
                ...
                Log.Info(string.Format("Balmung.Build {0}", versionString));
                ...
                // Load project
                {
                    var projectFiles = Directory.GetFiles(Globals.Root, "*.balproj", SearchOption.TopDirectoryOnly);
                ...
                {
                    singleInstanceMutex = new Mutex(true, "Balmung.Build", out var oneInstanceMutexCreated);
                    if (!oneInstanceMutexCreated)
                    {
                        try
                        {
                            ...
                            {
                                Log.Warning("Wait for another instance(s) of Balmung.Build to end...");
                                singleInstanceMutex.WaitOne();
                            }
                        }
                        catch (AbandonedMutexException)
                        {
                            // Can occur if another Balmung.Build is killed in the debugger
                        }
                        finally
                        {
