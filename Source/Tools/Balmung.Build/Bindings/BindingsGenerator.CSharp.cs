...
using System.Linq;
using System.Text;
using BuildData = Balmung.Build.Builder.BuildData;

namespace Balmung.Build.Bindings
{
    partial class BindingsGenerator
    {
        ...
        internal static readonly Dictionary<string, string> CSharpNativeToManagedDefault = new Dictionary<string, string>
        {
            ...
            { "ScriptingObject", "BalmungEngine.Object" },
            { "ScriptingTypeHandle", "System.Type" },
            { "PersistentScriptingObject", "BalmungEngine.Object" },
            { "ManagedScriptingObject", "BalmungEngine.Object" },
            { "Variant", "object" },
        ...
        private static string GenerateCSharpManagedToNativeConverter(BuildData buildData, TypeInfo typeInfo, ApiTypeInfo caller)
        {
            ...
            case "ManagedScriptingObject":
                // object
                return "BalmungEngine.Object.GetUnmanagedPtr({0})";
            case "Function":
                // delegate
            ...
            default:
                ...
                if (apiType != null)
                {
                    ...
                    if (apiType.IsInterface)
                        return string.Format("BalmungEngine.Object.GetUnmanagedInterface({{0}}, typeof({0}))", apiType.Name);
                }
        ...
        private static void GenerateCSharpWrapperFunction(BuildData buildData, StringBuilder contents, string indent, ApiTypeInfo caller, FunctionInfo functionInfo)
        {
            ...
            if (returnValueType == "bool")
                ...
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.SystemTypeMarshaller))";
            else if (returnValueType == "CultureInfo")
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.CultureInfoMarshaller))";
                ...
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.ManagedHandleMarshaller))";
            else if (FindApiTypeInfo(buildData, functionInfo.ReturnType, caller)?.IsInterface ?? false)
            {
                ...
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.SystemArrayMarshaller))";
            else if (returnValueType == "object[]")
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.SystemObjectArrayMarshaller))";
                ...
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.ArrayMarshaller<,>), CountElementName = nameof(__returnCount))";
            else if (functionInfo.ReturnType.Type == "Dictionary")
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.DictionaryMarshaller<,>), ConstantElementCount = 0)";
            else if (returnValueType == "byte[]")
                returnMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.ArrayMarshaller<,>), CountElementName = \"__returnCount\")";
            ...
            contents.AppendLine().Append(indent).Append($"[LibraryImport(\"{caller.ParentModule.Module.BinaryModuleName}\", EntryPoint = \"{functionInfo.Glue.LibraryEntryPoint}\", StringMarshalling = StringMarshalling.Custom, StringMarshallingCustomType = typeof(BalmungEngine.Interop.StringMarshaller))]");
            ...
            foreach (var parameterInfo in functionInfo.Parameters)
            {
                ...
                if (nativeType == "System.Type")
                    parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.SystemTypeMarshaller))";
                else if (parameterInfo.Type.Type == "CultureInfo")
                    parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.CultureInfoMarshaller))";
                else if (parameterInfo.Type.Type == "Version")
                    parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.VersionMarshaller))";
                else if (parameterInfo.Type.Type == "Variant") // object
                    parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.ManagedHandleMarshaller))";
                else if (parameterInfo.Type.Type == "MonoArray" || parameterInfo.Type.Type == "MArray")
                    parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.SystemArrayMarshaller))";
                else if (nativeType == "object[]")
                    parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.SystemObjectArrayMarshaller))";
                ...
                {
                    parameterMarshalType = $"MarshalUsing(typeof(BalmungEngine.Interop.ArrayMarshaller<,>), CountElementName = \"__{parameterInfo.Name}Count\")";
                    ...
                    parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.DictionaryMarshaller<,>), ConstantElementCount = 0)";
                ...
                {
                    parameterMarshalType = $"MarshalUsing(typeof(BalmungEngine.Interop.ArrayMarshaller<,>))";
                }
            ...
            for (var i = 0; i < customParametersCount; i++)
            {
                ...
                if (parameterInfo.IsOut && parameterInfo.DefaultValue == "var __resultAsRef")
                {
                    ...
                    if (parameterInfo.Type.Type == "Array" || parameterInfo.Type.Type == "Span" || parameterInfo.Type.Type == "DataContainer" || parameterInfo.Type.Type == "BytesContainer")
                        parameterMarshalType = $"MarshalUsing(typeof(BalmungEngine.Interop.ArrayMarshaller<,>), CountElementName = \"{parameterInfo.Name}Count\")";
                    else if (parameterInfo.Type.Type == "Dictionary")
                        parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.DictionaryMarshaller<,>), ConstantElementCount = 0)";
                    else if (parameterInfo.Type.Type == "CultureInfo")
                        parameterMarshalType = "MarshalUsing(typeof(BalmungEngine.Interop.CultureInfoMarshaller))";
        ...
        private static void GenerateCSharpClass(BuildData buildData, StringBuilder contents, string indent, ClassInfo classInfo)
        {
            ...
            foreach (var eventInfo in classInfo.Events)
            {
                ...
                contents.Append(indent).Append($"[LibraryImport(\"{classInfo.ParentModule.Module.BinaryModuleName}\", EntryPoint = \"{libraryEntryPoint}\", StringMarshalling = StringMarshalling.Custom, StringMarshallingCustomType = typeof(BalmungEngine.Interop.StringMarshaller))]").AppendLine();
        ...
        private static void GenerateCSharpStructure(BuildData buildData, StringBuilder contents, string indent, StructureInfo structureInfo)
        {
            ...
            if (UseCustomMarshalling(buildData, structureInfo, structureInfo))
            {
                ...
                // NOTE: Permanent BalmungEngine.Object GCHandles must not be released when marshalling from native to managed.
                    ...
                    foreach (var fieldInfo in structureInfo.Fields)
                    {
                        ...
                        if (marshalType.IsArray && (fieldInfo.NoArray || structureInfo.IsPod))
                        {
                            ...
                            if (GenerateCSharpUseFixedBuffer(originalType))
                            {
                                ...
                                toManagedContent.AppendLine($"BalmungEngine.Utils.MemoryCopy(new IntPtr(managed.{fieldInfo.Name}0), new IntPtr(unmanaged.{fieldInfo.Name}0), sizeof({originalType}) * {marshalType.ArraySize}ul);");
                                toNativeContent.AppendLine($"BalmungEngine.Utils.MemoryCopy(new IntPtr(unmanaged.{fieldInfo.Name}0), new IntPtr(managed.{fieldInfo.Name}0), sizeof({originalType}) * {marshalType.ArraySize}ul);");
                            }
                            else
#endif
                            {
                        ...
                        {
                            toManagedContent.AppendLine($"unmanaged.{fieldInfo.Name} != IntPtr.Zero ? Unsafe.As<BalmungEngine.Object>(ManagedHandle.FromIntPtr(unmanaged.{fieldInfo.Name}).Target) : null;");
                            toNativeContent.AppendLine($"managed.{fieldInfo.Name} != null ? ManagedHandle.ToIntPtr(managed.{fieldInfo.Name}, GCHandleType.Weak) : IntPtr.Zero;");
                            freeContents.AppendLine($"if (unmanaged.{fieldInfo.Name} != IntPtr.Zero) {{ ManagedHandle.FromIntPtr(unmanaged.{fieldInfo.Name}).Free(); }}");
            ...
            if (!structureInfo.NoDefault && !hasDefaultMember)
            {
                ...
                if (useDefaultValueInit)
                {
                    ...
                    contents.Append(indent).AppendLine($"BalmungEngine.Utils.InitStructure(obj, typeof({structureInfo.Name}));");
        ...
        private static void GenerateCSharp(BuildData buildData, ModuleInfo moduleInfo, ref BindingsResult bindings)
        {
            ...
            CSharpUsedNamespaces.Add("BalmungEngine.Interop");
