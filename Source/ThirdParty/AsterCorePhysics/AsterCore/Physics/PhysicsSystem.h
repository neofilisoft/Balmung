...
class ACPH_EXPORT PhysicsSystem : public NonCopyable
{
public:
	ACPH_OVERRIDE_NEW_DELETE

	/// Constructor / Destructor
								PhysicsSystem()												: mContactManager(mPhysicsSettings) ACPH_IF_ENABLE_ASSERTS(, mConstraintManager(&mBodyManager)) { }
								~PhysicsSystem();
	...
	void						SetComputeScheduler(PhysicsComputeScheduler *inScheduler) { mComputeScheduler = inScheduler; }
	PhysicsComputeScheduler *	GetComputeScheduler() const { return mComputeScheduler; }
...
private:
	...
	/// Optional compute scheduler that can offload selected broad phase / narrow phase work to an accelerator.
	PhysicsComputeScheduler *	mComputeScheduler = nullptr;
