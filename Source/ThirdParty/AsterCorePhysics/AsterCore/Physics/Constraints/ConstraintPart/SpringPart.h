...
#ifndef ACPH_PLATFORM_DOXYGEN // Somehow Doxygen gets confused and thinks the parameters to CalculateSpringProperties belong to this macro
ACPH_MSVC_SUPPRESS_WARNING(4723) // potential divide by 0 - caused by line: outEffectiveMass = 1.0f / inInvEffectiveMass, note that ACPH_NAMESPACE_BEGIN already pushes the warning state
#endif // !ACPH_PLATFORM_DOXYGEN
...
{
private:
	ACPH_INLINE void				CalculateSpringPropertiesHelper(float inDeltaTime, float inInvEffectiveMass, float inBias, float inC, float inStiffness, float inDamping, float &outEffectiveMass)
	{
...
public:
	...
	inline void					CalculateSpringPropertiesWithFrequencyAndDamping(float inDeltaTime, float inInvEffectiveMass, float inBias, float inC, float inFrequency, float inDamping, float &outEffectiveMass)
	{
		...
		if (inFrequency > 0.0f)
		{
			...
			float omega = 2.0f * ACPH_PI * inFrequency;
