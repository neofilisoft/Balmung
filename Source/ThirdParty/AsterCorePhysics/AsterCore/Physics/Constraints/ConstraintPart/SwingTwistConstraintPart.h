...
public:
	...
	void						SetLimits(float inTwistMinAngle, float inTwistMaxAngle, float inSwingYMinAngle, float inSwingYMaxAngle, float inSwingZMinAngle, float inSwingZMaxAngle)
	{
		...
		// Assume sane input
		ACPH_ASSERT(inTwistMinAngle <= inTwistMaxAngle);
		ACPH_ASSERT(inSwingYMinAngle <= inSwingYMaxAngle);
		ACPH_ASSERT(inSwingZMinAngle <= inSwingZMaxAngle);
		ACPH_ASSERT(inSwingYMinAngle >= -ACPH_PI && inSwingYMaxAngle <= ACPH_PI);
		ACPH_ASSERT(inSwingZMinAngle >= -ACPH_PI && inSwingZMaxAngle <= ACPH_PI);
		...
		else
		{
			...
			ACPH_ASSERT(mSinSwingYHalfMinAngle <= mSinSwingYHalfMaxAngle);
		}
		...
		else
		{
			...
			ACPH_ASSERT(mSinSwingZHalfMinAngle <= mSinSwingZHalfMaxAngle);
		}
	}
	...
	static ACPH_INLINE bool		sDistanceToMinShorter(float inDeltaMin, float inDeltaMax)
	{
	...
	inline void					ClampSwingTwist(Quat &ioSwing, Quat &ioTwist, uint &outClampedAxis) const
	{
		...
		ACPH_ASSERT(ioSwing.GetX() == 0.0f);
		ACPH_ASSERT(ioTwist.GetY() == 0.0f);
		ACPH_ASSERT(ioTwist.GetZ() == 0.0f);
		...
		ACPH_ASSERT(ioSwing.IsNormalized());
		ACPH_ASSERT(ioTwist.IsNormalized());
	}
