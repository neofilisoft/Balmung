...
#include <AsterCore/Physics/Constraints/ConstraintPart/SpringPart.h>
...
class AxisConstraintPart
{
	...
	ACPH_INLINE bool				ApplyVelocityStep(MotionProperties *ioMotionProperties1, float inInvMass1, MotionProperties *ioMotionProperties2, float inInvMass2, Vec3Arg inWorldSpaceAxis, float inLambda) const
	{
	...
	template <EMotionType Type1, EMotionType Type2>
	ACPH_INLINE float			TemplatedCalculateInverseEffectiveMass(float inInvMass1, Mat44Arg inInvI1, Vec3Arg inR1PlusU, float inInvMass2, Mat44Arg inInvI2, Vec3Arg inR2, Vec3Arg inWorldSpaceAxis)
	{
		ACPH_ASSERT(inWorldSpaceAxis.IsNormalized(1.0e-5f));
		...
		else
		{
			...
			ACPH_IF_DEBUG(Vec3::sNaN().StoreFloat3(&mInvI1_R1PlusUxAxis);)
		...
		else
		{
			...
			ACPH_IF_DEBUG(Vec3::sNaN().StoreFloat3(&mInvI2_R2xAxis);)
		}
	...
	/// Internal helper function to calculate the inverse effective mass
	ACPH_INLINE float			CalculateInverseEffectiveMass(const Body &inBody1, Vec3Arg inR1PlusU, const Body &inBody2, Vec3Arg inR2, Vec3Arg inWorldSpaceAxis)
	{
	...
	ACPH_INLINE float			CalculateInverseEffectiveMassWithMassOverride(const Body &inBody1, float inInvMass1, float inInvInertiaScale1, Vec3Arg inR1PlusU, const Body &inBody2, float inInvMass2, float inInvInertiaScale2, Vec3Arg inR2, Vec3Arg inWorldSpaceAxis)
	{
		// Dispatch to the correct templated form
		switch (inBody1.GetMotionType())
		{
...
public:
	...
	template <EMotionType Type1, EMotionType Type2>
	ACPH_INLINE void				TemplatedCalculateConstraintProperties(float inInvMass1, Mat44Arg inInvI1, Vec3Arg inR1PlusU, float inInvMass2, Mat44Arg inInvI2, Vec3Arg inR2, Vec3Arg inWorldSpaceAxis, float inBias = 0.0f)
	{
	...
	template <EMotionType Type1, EMotionType Type2>
	ACPH_INLINE float			TemplatedSolveVelocityConstraintGetTotalLambda(const MotionProperties *ioMotionProperties1, const MotionProperties *ioMotionProperties2, Vec3Arg inWorldSpaceAxis) const
	{
		...
		if constexpr (Type1 != EMotionType::Static && Type2 != EMotionType::Static)
			...
			ACPH_ASSERT(false); // Static vs static is nonsensical!
	...
	template <EMotionType Type1, EMotionType Type2>
	ACPH_INLINE bool				TemplatedSolveVelocityConstraintApplyLambda(MotionProperties *ioMotionProperties1, float inInvMass1, MotionProperties *ioMotionProperties2, float inInvMass2, Vec3Arg inWorldSpaceAxis, float inTotalLambda)
	{
