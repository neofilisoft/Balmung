...
//#define ACPH_LARGE_ISLAND_SPLITTER_DEBUG
...
{
	{
		...
		else
		{
			...
			ACPH_ASSERT(split_index < mNumSplits);
	...
	uint split_index = sGetSplit(status);
	ACPH_ASSERT(split_index < mNumSplits || split_index == cNonParallelSplitIdx);
	const Split &split = mSplits[split_index];
...
void LargeIslandSplitter::Splits::MarkBatchProcessed(uint inNumProcessed, bool &outLastIteration, bool &outFinalBatch)
{
	...
	ACPH_ASSERT(inNumProcessed > 0); // Logic will break if we mark a block of 0 items as processed
	...
	{
		ACPH_ASSERT(total_items_processed == num_items_in_split); // Should not overflow, that means we're retiring more items than we should process
...
{
	ACPH_ASSERT(mSplitMasks == nullptr);
	ACPH_ASSERT(mContactAndConstraintsSplitIdx == nullptr);
	ACPH_ASSERT(mContactAndConstraintIndices == nullptr);
	ACPH_ASSERT(mSplitIslands == nullptr);
}
...
uint LargeIslandSplitter::AssignSplit(const Body *inBody1, const Body *inBody2)
{
	...
	if (idx1 == Body::cInactiveIndex || !inBody1->IsDynamic())
	{
		...
		ACPH_ASSERT(idx2 < mNumActiveBodies);
	...
	else if (idx2 == Body::cInactiveIndex || !inBody2->IsDynamic())
	{
		...
		ACPH_ASSERT(idx1 < mNumActiveBodies);
...
uint LargeIslandSplitter::AssignToNonParallelSplit(const Body *inBody)
{
	...
	{
		ACPH_ASSERT(idx < mNumActiveBodies);
...
bool LargeIslandSplitter::SplitIsland(uint32 inIslandIndex, const IslandBuilder &inIslandBuilder, const BodyManager &inBodyManager, const ContactConstraintManager &inContactManager, Constraint **inActiveConstraints, CalculateSolverSteps &ioStepsCalculator)
{
	...
	ACPH_ASSERT(new_split_idx < mNumSplitIslands);
...
#ifdef ACPH_LARGE_ISLAND_SPLITTER_DEBUG
	...
	Trace(stats.c_str());
#endif // ACPH_LARGE_ISLAND_SPLITTER_DEBUG
	...
	for (uint s = 0; s < cNumSplits; ++s)
	{
		...
		ACPH_ASSERT(contact_buffer_cur[s] == mContactAndConstraintIndices + split.mContactBufferEnd);
		ACPH_ASSERT(constraint_buffer_cur[s] == mContactAndConstraintIndices + split.mConstraintBufferEnd);
	}
...
void LargeIslandSplitter::MarkBatchProcessed(uint inSplitIslandIndex, const uint32 *inConstraintsBegin, const uint32 *inConstraintsEnd, const uint32 *inContactsBegin, const uint32 *inContactsEnd, bool &outLastIteration, bool &outFinalBatch)
{
	...
	ACPH_ASSERT(inSplitIslandIndex < mNextSplitIsland.load(memory_order_relaxed));
...
void LargeIslandSplitter::Reset(TempAllocator *inTempAllocator)
{
	...
	ACPH_ASSERT(mContactAndConstraintsNextFree.load(memory_order_relaxed) == mContactAndConstraintsSize);
	ACPH_ASSERT(mNextSplitIsland.load(memory_order_relaxed) == mNumSplitIslands);
