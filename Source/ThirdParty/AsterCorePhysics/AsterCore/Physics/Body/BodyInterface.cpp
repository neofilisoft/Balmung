...
bool BodyInterface::HasBody(const BodyID &inBodyID) const
{
	BodyLockRead lock(*mBodyLockInterface, inBodyID);
	return lock.Succeeded();
}

bool BodyInterface::IsBodyIDAvailable(const BodyID &inBodyID) const
{
	return mBodyManager->IsBodyIDAvailable(inBodyID);
}

uint BodyInterface::GetMaxBodies() const
{
...
BodyCreationSettings BodyInterface::GetBodyCreationSettings(const BodyID &inBodyID) const
{
	BodyCreationSettings settings;
	BodyLockRead lock(*mBodyLockInterface, inBodyID);
	if (lock.Succeeded() && lock.GetBody().IsRigidBody())
