...
bool BodyManager::IsBodyIDAvailable(const BodyID &inID) const
{
	UniqueLock lock(mBodiesMutex ACPH_IF_ENABLE_ASSERTS(, this, EPhysicsLockTypes::BodiesList));

	const uint32 idx = inID.GetIndex();
	if (idx >= mBodies.capacity())
		return false;

	return idx >= mBodies.size() || !sIsValidBodyPointer(mBodies[idx]);
}
...
{
	{
		...
		std::sort(bodies.begin(), bodies.end(), [](const Body *inLHS, const Body *inRHS)
		{
			return inLHS->GetID() < inRHS->GetID();
		});
...
bool BodyManager::RestoreState(StateRecorder &inStream)
{
...

	{
		...
		{
			// Read state of bodies in the same deterministic BodyID order as SaveState.
			Array<Body *> bodies;
			bodies.reserve(mNumBodies);
			for (Body *b : mBodies)
			...
			uint32 old_num_bodies = uint32(bodies.size());
