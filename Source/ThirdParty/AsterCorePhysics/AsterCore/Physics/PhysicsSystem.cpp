...
void PhysicsSystem::Init(uint inMaxBodies, uint inNumBodyMutexes, uint inMaxBodyPairs, uint inMaxContactConstraints, const BroadPhaseLayerInterface &inBroadPhaseLayerInterface, const ObjectVsBroadPhaseLayerFilter &inObjectVsBroadPhaseLayerFilter, const ObjectLayerPairFilter &inObjectLayerPairFilter)
{
	...
	ACPH_ASSERT(max_bodies == inMaxBodies, "Cannot support this many bodies!");
...
void PhysicsSystem::AddStepListener(PhysicsStepListener *inListener)
{
	...
	ACPH_ASSERT(std::find(mStepListeners.begin(), mStepListeners.end(), inListener) == mStepListeners.end());
...
void PhysicsSystem::RemoveStepListener(PhysicsStepListener *inListener)
{
	...
	ACPH_ASSERT(i != mStepListeners.end());
...
#endif // ACPH_TRACK_SIMULATION_STATS
...
EPhysicsUpdateError PhysicsSystem::Update(float inDeltaTime, int inCollisionSteps, TempAllocator *inTempAllocator, JobSystem *inJobSystem)
{
	...
	ACPH_DET_LOG("PhysicsSystem::Update: dt: " << inDeltaTime << " steps: " << inCollisionSteps);

	ACPH_ASSERT(inCollisionSteps > 0);
	ACPH_ASSERT(inDeltaTime >= 0.0f);
	...
	ACPH_ASSERT(context.mBodyPairs == nullptr);
...

	{
		...
		for (int step_idx = 0; step_idx < inCollisionSteps; ++step_idx)
		{
			...
			step.mUpdateBroadphaseFinalize = inJobSystem->CreateJob("UpdateBroadPhaseFinalize", cColorUpdateBroadPhaseFinalize, [&context, &step]()
				{
					...
					ACPH_ASSERT(step.mActiveFindCollisionJobs.load(memory_order_relaxed) == 0);
			...
			if (is_first_step)
			{
				...
				ACPH_ASSERT(context.mActiveConstraints == nullptr);
	...
	{
		ACPH_PROFILE("Build job barrier");
	...
	ACPH_ASSERT(errors == EPhysicsUpdateError::None, "An error occurred during the physics update, see EPhysicsUpdateError for more information");
	return errors;
}
...
void PhysicsSystem::JobDetermineActiveConstraints(PhysicsUpdateContext::Step *ioStep) const
{
	...
	Constraint **active_constraints = (Constraint **)ACPH_STACK_ALLOC(cDetermineActiveConstraintsBatchSize * sizeof(Constraint *));
...
void PhysicsSystem::JobFindCollisions(PhysicsUpdateContext::Step *ioStep, int inJobIndex)
{
	...
	BodyID *active_bodies = (BodyID *)ACPH_STACK_ALLOC(cActiveBodiesBatchSize * sizeof(BodyID));
...
void PhysicsSystem::ProcessBodyPair(ContactAllocator &ioContactAllocator, const BodyPair &inBodyPair)
{
	...
	ACPH_ASSERT(body1->IsActive());

	ACPH_DET_LOG("ProcessBodyPair: id1: " << inBodyPair.mBodyA << " id2: " << inBodyPair.mBodyB << " p1: " << body1->GetCenterOfMassPosition() << " p2: " << body2->GetCenterOfMassPosition() << " r1: " << body1->GetRotation() << " r2: " << body2->GetRotation());
	...
	if (!pair_handled)
	{
			...
			&& body1->GetUseManifoldReductionWithBody(*body2))	// Check body flag
		{
			...
			public:
				...
				virtual void	AddHit(const CollideShapeResult &inResult) override
				{
					...
					ACPH_ASSERT(mBody1->GetMotionType() >= mBody2->GetMotionType());
					ACPH_ASSERT(!ShouldEarlyOut());
					...
					ManifoldBetweenTwoFaces(inResult.mContactPointOn1, inResult.mContactPointOn2, inResult.mPenetrationAxis, settings.mSpeculativeContactDistance + settings.mManifoldTolerance, inResult.mShape1Face, inResult.mShape2Face, manifold->mRelativeContactPointsOn1, manifold->mRelativeContactPointsOn2 ACPH_IF_DEBUG_RENDERER(, mBody1->GetCenterOfMassPosition()));
					...
					if (manifold->mRelativeContactPointsOn1.size() > 32)
						PruneContactPoints(manifold->mFirstWorldSpaceNormal, manifold->mRelativeContactPointsOn1, manifold->mRelativeContactPointsOn2 ACPH_IF_DEBUG_RENDERER(, manifold->mBaseOffset));
				}
			...
			for (ContactManifold &manifold : collector.mManifolds)
			{
				...
				if (manifold.mRelativeContactPointsOn1.size() > 4)
					PruneContactPoints(manifold.mWorldSpaceNormal, manifold.mRelativeContactPointsOn1, manifold.mRelativeContactPointsOn2 ACPH_IF_DEBUG_RENDERER(, manifold.mBaseOffset));
		...
		else
		{
			...
			public:
				...
				virtual void	AddHit(const CollideShapeResult &inResult) override
				{
					...
					const PhysicsSettings &settings = mSystem->mPhysicsSettings;
					ManifoldBetweenTwoFaces(inResult.mContactPointOn1, inResult.mContactPointOn2, inResult.mPenetrationAxis, settings.mSpeculativeContactDistance + settings.mManifoldTolerance, inResult.mShape1Face, inResult.mShape2Face, manifold.mRelativeContactPointsOn1, manifold.mRelativeContactPointsOn2 ACPH_IF_DEBUG_RENDERER(, manifold.mBaseOffset));
		...
		else
		{
			...
			ACPH_ASSERT(body1->GetMotionType() == body2->GetMotionType());
			num_ticks /= 2;
			body1->GetMotionProperties()->GetSimulationStats().mNarrowPhaseTicks.fetch_add(num_ticks, memory_order_relaxed);
			body1->GetMotionProperties()->GetSimulationStats().mNarrowPhaseTicks.fetch_add(num_ticks, memory_order_relaxed);
		}
...
ACPH_CLANG_SUPPRESS_WARNING("-Wundefined-func-template") // ConstraintManager::sWarmStartVelocityConstraints / ContactConstraintManager::WarmStartVelocityConstraints is instantiated in the cpp file
	...
	for (;;)
	{
		...
		if (check_islands)
		{
			...
			if (!has_contacts && !has_constraints)
			{
				...
				{
					ACPH_ASSERT(!mIslandBuilder.GetConstraintsInIsland(island_idx, constraints_begin, constraints_end));
					ACPH_ASSERT(!mIslandBuilder.GetContactsInIsland(island_idx, contacts_begin, contacts_end));
				}
...
void PhysicsSystem::JobPreIntegrateVelocity(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	ACPH_ASSERT(ioStep->mCCDBodies == nullptr);
	...
	ACPH_ASSERT(ioStep->mActiveBodyToCCDBody == nullptr);
...
void PhysicsSystem::JobIntegrateVelocity(const PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	BodyID *bodies_to_update_bounds = (BodyID *)ACPH_STACK_ALLOC(cBodiesBatch * sizeof(BodyID));
	...
	for (;;)
	{
		...
		while (active_body_idx < active_body_idx_end)
		{
			...
			ACPH_DET_LOG("JobIntegrateVelocity: id: " << body_id << " v: " << body.GetLinearVelocity() << " w: " << body.GetAngularVelocity());
			...
			case EMotionQuality::LinearCast:
					...
					&& !body.IsSensor()) // We don't support CCD sensors
				{
					...
					ACPH_ASSERT(inner_radius > 0.0f, "The shape has no inner radius, this makes the shape unsuitable for the linear cast motion quality as we cannot move it without risking tunneling.");
					...
					if (delta_pos.LengthSq() > linear_cast_threshold_sq)
					{
						...
						ACPH_ASSERT(active_body_idx < ioStep->mNumActiveBodyToCCDBody);
...
void PhysicsSystem::JobPostIntegrateVelocity(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep) const
{
	...
	ACPH_ASSERT(ioStep->mNumCCDBodies <= mBodyManager.GetNumActiveCCDBodies());
...
inline static PhysicsUpdateContext::Step::CCDBody *sGetCCDBody(const Body &inBody, PhysicsUpdateContext::Step *inStep)
{
	...
	ACPH_ASSERT(active_index < inStep->mNumActiveBodyToCCDBody); // Ensure that the body has a mapping to CCD body
	...
	ACPH_ASSERT(ccd_body->mBodyID1 == inBody.GetID(), "We found the wrong CCD body!");
	return ccd_body;
}
	...
	for (;;)
	{
		...
		public:
			...
			virtual void				AddHit(const BroadPhaseCastResult &inResult) override
			{
				...
				ACPH_ASSERT(inResult.mFraction <= GetEarlyOutFraction(), "This hit should not have been passed on to the collector");
...
void PhysicsSystem::JobResolveCCDContacts(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	if (num_ccd_bodies > 0)
	{
		...
		ACPH_SCOPE_EXIT([temp_allocator, sorted_ccd_bodies, num_ccd_bodies]{ temp_allocator->Free(sorted_ccd_bodies, num_ccd_bodies * sizeof(CCDBody *)); });
		{
			ACPH_PROFILE("Sort");
		...
		static constexpr int cBodiesBatch = 64;
		BodyID *bodies_to_activate = (BodyID *)ACPH_STACK_ALLOC(cBodiesBatch * sizeof(BodyID));
		...
		for (uint i = 0; i < num_ccd_bodies; ++i)
		{
			...
			if (!ccd_body->mBodyID2.IsInvalid())
			{
				...
				{
					ACPH_ASSERT(ccd_body2->mBodyID2 != ccd_body->mBodyID1, "If we collided with another body, that other body should have ignored collisions with us!");
...
void PhysicsSystem::JobSolvePositionConstraints(PhysicsUpdateContext *ioContext, PhysicsUpdateContext::Step *ioStep)
{
	...
	BodiesToSleep bodies_to_sleep(mBodyManager, (BodyID *)ACPH_STACK_ALLOC(BodiesToSleep::cBodiesToSleepSize * sizeof(BodyID)));
...
void PhysicsSystem::JobSoftBodyFinalize(PhysicsUpdateContext *ioContext)
{
	...
	BodyID *bodies_to_update_bounds = (BodyID *)ACPH_STACK_ALLOC(cBodiesBatch * sizeof(BodyID));
	int num_bodies_to_update_bounds = 0;
	BodyID *bodies_to_put_to_sleep = (BodyID *)ACPH_STACK_ALLOC(cBodiesBatch * sizeof(BodyID));
