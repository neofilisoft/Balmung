...
//#define ACPH_VALIDATE_ISLAND_BUILDER

/// Keeps track of connected bodies and builds islands for multithreaded velocity/position update
class ACPH_EXPORT IslandBuilder : public NonCopyable
{
public:
	/// Destructor
							~IslandBuilder();
	...
	void					SetNumPositionSteps(uint32 inIslandIndex, uint inNumPositionSteps)	{ ACPH_ASSERT(inIslandIndex < mNumIslands); ACPH_ASSERT(inNumPositionSteps < 256); mNumPositionSteps[inIslandIndex] = uint8(inNumPositionSteps); }
	uint					GetNumPositionSteps(uint32 inIslandIndex) const						{ ACPH_ASSERT(inIslandIndex < mNumIslands); return mNumPositionSteps[inIslandIndex]; }
