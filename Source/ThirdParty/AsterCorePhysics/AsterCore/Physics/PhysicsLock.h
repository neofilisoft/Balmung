...
public:
	...
	static inline void			sCheckLock(PhysicsLockContext inContext, EPhysicsLockTypes inType)
	{
		...
		ACPH_ASSERT(uint32(inType) > mutexes, "A lock of same or higher priority was already taken, this can create a deadlock!");
	...
	{
		uint32 &mutexes = sGetLockedMutexes(inContext);
		ACPH_ASSERT((mutexes & uint32(inType)) != 0, "Mutex was not locked!");
		mutexes = mutexes & ~uint32(inType);
	}
#endif // !ACPH_ENABLE_ASSERTS

	template <class LockType>
	static inline void			sLock(LockType &inMutex ACPH_IF_ENABLE_ASSERTS(, PhysicsLockContext inContext, EPhysicsLockTypes inType))
	{
		ACPH_IF_ENABLE_ASSERTS(sCheckLock(inContext, inType);)
		inMutex.lock();
	}

	template <class LockType>
	static inline void			sUnlock(LockType &inMutex ACPH_IF_ENABLE_ASSERTS(, PhysicsLockContext inContext, EPhysicsLockTypes inType))
	{
		ACPH_IF_ENABLE_ASSERTS(sCheckUnlock(inContext, inType);)
		inMutex.unlock();
	}

	template <class LockType>
	static inline void			sLockShared(LockType &inMutex ACPH_IF_ENABLE_ASSERTS(, PhysicsLockContext inContext, EPhysicsLockTypes inType))
	{
		ACPH_IF_ENABLE_ASSERTS(sCheckLock(inContext, inType);)
		inMutex.lock_shared();
	}
	...
	template <class LockType>
	static inline void			sUnlockShared(LockType &inMutex ACPH_IF_ENABLE_ASSERTS(, PhysicsLockContext inContext, EPhysicsLockTypes inType))
	{
		ACPH_IF_ENABLE_ASSERTS(sCheckUnlock(inContext, inType);)
...
private:
	...
	static uint32 &				sGetLockedMutexes(PhysicsLockContext inContext)
	{
		...
		ACPH_ASSERT(false, "Too many physics systems locked at the same time!");
...
{
public:
	explicit					UniqueLock(LockType &inLock ACPH_IF_ENABLE_ASSERTS(, PhysicsLockContext inContext, EPhysicsLockTypes inType)) :
		mLock(inLock)
#ifdef ACPH_ENABLE_ASSERTS
		, mContext(inContext),
		mType(inType)
#endif // ACPH_ENABLE_ASSERTS
	{
		PhysicsLock::sLock(mLock ACPH_IF_ENABLE_ASSERTS(, mContext, mType));
	}

								~UniqueLock()
	{
		PhysicsLock::sUnlock(mLock ACPH_IF_ENABLE_ASSERTS(, mContext, mType));
	}
...
{
public:
	explicit					SharedLock(LockType &inLock ACPH_IF_ENABLE_ASSERTS(, PhysicsLockContext inContext, EPhysicsLockTypes inType)) :
		mLock(inLock)
#ifdef ACPH_ENABLE_ASSERTS
		, mContext(inContext)
		, mType(inType)
#endif // ACPH_ENABLE_ASSERTS
	{
		PhysicsLock::sLockShared(mLock ACPH_IF_ENABLE_ASSERTS(, mContext, mType));
	}

								~SharedLock()
	{
		PhysicsLock::sUnlockShared(mLock ACPH_IF_ENABLE_ASSERTS(, mContext, mType));
	}
