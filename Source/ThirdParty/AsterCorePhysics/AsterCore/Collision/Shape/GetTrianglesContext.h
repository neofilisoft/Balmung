...
public:
		...
		mIsInsideOut(ScaleHelpers::IsInsideOut(inScale))
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(GetTrianglesContextVertexList)));
		ACPH_ASSERT(inNumTriangleVertices % 3 == 0);
	}

	/// @see Shape::GetTrianglesNext
	int				GetTrianglesNext(int inMaxTrianglesRequested, Float3 *outTriangleVertices, const PhysicsMaterial **outMaterials)
	{
		ACPH_ASSERT(inMaxTrianglesRequested >= Shape::cGetTrianglesMinTrianglesRequested);
	...
	static void		sCreateUnitOpenCylinder(A &ioVertices, int inDetailLevel)
	{
		...
		{
			float angle1 = 2.0f * ACPH_PI * (float(i) / num_verts);
			float angle2 = 2.0f * ACPH_PI * (float(i + 1) / num_verts);
...
public:
		...
		mIsInsideOut(inIsInsideOut)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(GetTrianglesContextMultiVertexList)));
	}
