...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(SphereShapeSettings)
{
	ACPH_ADD_BASE_CLASS(SphereShapeSettings, ConvexShapeSettings)

	ACPH_ADD_ATTRIBUTE(SphereShapeSettings, mRadius)
}
...
public:
		...
		mRadius(inRadius)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(SphereNoConvex)));
	}
...
public:
		...
		mRadius(inRadius)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(SphereWithConvex)));
	}
...
MassProperties SphereShape::GetMassProperties() const
{
	...
	p.mMass = (4.0f / 3.0f * ACPH_PI) * mRadius * r2 * GetDensity();
...
void SphereShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	float scaled_radius = GetScaledRadius(inScale);
	outTotalVolume = (4.0f / 3.0f * ACPH_PI) * Cubed(scaled_radius);
	...
	else
	{
		...
		outSubmergedVolume = (ACPH_PI / 3.0f) * Square(h) * (3.0f * scaled_radius - h);
		...
		if (sDrawSubmergedVolumes)
		{
			...
			DebugRenderer::sInstance->DrawPie(inBaseOffset + circle_center, circle_radius, inSurface.GetNormal(), inSurface.GetNormal().GetNormalizedPerpendicular(), -ACPH_PI, ACPH_PI, Color::sGreen, DebugRenderer::ECastShadow::Off);
		}
	#endif // ACPH_DEBUG_RENDERER
	}
