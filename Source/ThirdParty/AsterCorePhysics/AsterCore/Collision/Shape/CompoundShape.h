...
class ACPH_EXPORT CompoundShapeSettings : public ShapeSettings
{
	ACPH_DECLARE_SERIALIZABLE_ABSTRACT(ACPH_EXPORT, CompoundShapeSettings)
...
public:
	...
	{
		ACPH_DECLARE_SERIALIZABLE_NON_VIRTUAL(ACPH_EXPORT, SubShapeSettings)
...
class ACPH_EXPORT CompoundShape : public Shape
{
public:
	...
	virtual void					GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const override;
	...
	virtual void					GetTrianglesStart(GetTrianglesContext &ioContext, const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale) const override { ACPH_ASSERT(false, "Cannot call on non-leaf shapes, use CollectTransformedShapes to collect the leaves first!"); }

	// See Shape::GetTrianglesNext
	virtual int						GetTrianglesNext(GetTrianglesContext &ioContext, int inMaxTrianglesRequested, Float3 *outTriangleVertices, const PhysicsMaterial **outMaterials = nullptr) const override { ACPH_ASSERT(false, "Cannot call on non-leaf shapes, use CollectTransformedShapes to collect the leaves first!"); return 0; }
	...
	struct SubShape
	{
		...
		ACPH_INLINE void				SetTransform(Vec3Arg inPosition, QuatArg inRotation, Vec3Arg inCenterOfMass)
		{
		...
		ACPH_INLINE Mat44			GetLocalTransformNoScale(Vec3Arg inScale) const
		{
	...
	inline uint32					GetSubShapeIndexFromID(SubShapeID inSubShapeID, SubShapeID &outRemainder) const
	{
		...
		ACPH_ASSERT(idx < mSubShapes.size(), "Invalid SubShapeID");
		return idx;
	}
