...
ACPH_IMPLEMENT_SERIALIZABLE_ABSTRACT_BASE(ShapeSettings)
{
	ACPH_ADD_BASE_CLASS(ShapeSettings, SerializableObject)

	ACPH_ADD_ATTRIBUTE(ShapeSettings, mUserData)
}
...
Shape::ShapeResult Shape::sRestoreWithChildren(StreamIn &inStream, IDToShapeMap &ioShapeMap, IDToMaterialMap &ioMaterialMap)
{
	...
	if (result.HasError())
		return result;
	ACPH_ASSERT(ioShapeMap.size() == shape_id); // Assert that this is the next ID in the map
