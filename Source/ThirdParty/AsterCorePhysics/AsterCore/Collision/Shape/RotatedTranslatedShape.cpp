...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(RotatedTranslatedShapeSettings)
{
	ACPH_ADD_BASE_CLASS(RotatedTranslatedShapeSettings, DecoratedShapeSettings)

	ACPH_ADD_ATTRIBUTE(RotatedTranslatedShapeSettings, mPosition)
	ACPH_ADD_ATTRIBUTE(RotatedTranslatedShapeSettings, mRotation)
}
...
void RotatedTranslatedShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	// Get center of mass transform of child
	Mat44 transform = inCenterOfMassTransform * Mat44::sRotation(mRotation);

	// Recurse to child
	mInnerShape->GetSubmergedVolume(transform, TransformScale(inScale), inSurface, outTotalVolume, outSubmergedVolume, outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, inBaseOffset));
}
...
{
	ACPH_ASSERT(inShape1->GetSubType() == EShapeSubType::RotatedTranslated);
...
{
	ACPH_ASSERT(inShape2->GetSubType() == EShapeSubType::RotatedTranslated);
...
void RotatedTranslatedShape::sCastRotatedTranslatedVsShape(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	ACPH_ASSERT(inShapeCast.mShape->GetSubType() == EShapeSubType::RotatedTranslated);
...
{
	ACPH_ASSERT(inShape->GetSubType() == EShapeSubType::RotatedTranslated);
