...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(TaperedCylinderShapeSettings)
{
	ACPH_ADD_BASE_CLASS(TaperedCylinderShapeSettings, ConvexShapeSettings)

	ACPH_ADD_ATTRIBUTE(TaperedCylinderShapeSettings, mHalfHeight)
	ACPH_ADD_ATTRIBUTE(TaperedCylinderShapeSettings, mTopRadius)
	ACPH_ADD_ATTRIBUTE(TaperedCylinderShapeSettings, mBottomRadius)
	ACPH_ADD_ATTRIBUTE(TaperedCylinderShapeSettings, mConvexRadius)
}
...
public:
		...
		mConvexRadius(inConvexRadius)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(TaperedCylinder)));
	}
...
ACPH_INLINE void TaperedCylinderShape::GetScaled(Vec3Arg inScale, float &outTop, float &outBottom, float &outTopRadius, float &outBottomRadius, float &outConvexRadius) const
{
...
ACPH_INLINE static Vec3 sCalculateSideNormalXZ(Vec3Arg inSurfacePosition)
{
	return (Vec3(1, 0, 1) * inSurfacePosition).NormalizedOr(Vec3::sAxisX());
}

ACPH_INLINE static Vec3 sCalculateSideNormal(Vec3Arg inNormalXZ, float inTop, float inBottom, float inTopRadius, float inBottomRadius)
{
...
MassProperties TaperedCylinderShape::GetMassProperties() const
{
	...
	float inertia_y = (ACPH_PI / 10.0f) * density * (t - b) * (br4 + tr * br3 + tr2 * br2 + tr3 * br + tr4);
	float inertia_x_delta = (ACPH_PI / 30.0f) * density * ((t3 + 2 * b * t2 + 3 * b2 * t - 6 * b3) * br2 + (3 * t3 + b * t2 - b2 * t - 3 * b3) * tr * br + (6 * t3 - 3 * b * t2 - 2 * b2 * t - b3) * tr2);
...
void TaperedCylinderShape::GetTrianglesStart(GetTrianglesContext &ioContext, const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale) const
{
	...
	ACPH_ASSERT(IsAligned(&ioContext, alignof(TCSGetTrianglesContext)));
...
float TaperedCylinderShape::GetVolume() const
{
	...
	return (ACPH_PI / 3.0f) * (mTop - mBottom) * (Square(mTopRadius) + mTopRadius * mBottomRadius + Square(mBottomRadius));
}
