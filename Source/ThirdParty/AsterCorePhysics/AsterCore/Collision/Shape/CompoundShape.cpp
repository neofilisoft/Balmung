...
ACPH_IMPLEMENT_SERIALIZABLE_ABSTRACT(CompoundShapeSettings)
{
	ACPH_ADD_BASE_CLASS(CompoundShapeSettings, ShapeSettings)

	ACPH_ADD_ATTRIBUTE(CompoundShapeSettings, mSubShapes)
}

ACPH_IMPLEMENT_SERIALIZABLE_NON_VIRTUAL(CompoundShapeSettings::SubShapeSettings)
{
	ACPH_ADD_ATTRIBUTE(CompoundShapeSettings::SubShapeSettings, mShape)
	ACPH_ADD_ATTRIBUTE(CompoundShapeSettings::SubShapeSettings, mPosition)
	ACPH_ADD_ATTRIBUTE(CompoundShapeSettings::SubShapeSettings, mRotation)
	ACPH_ADD_ATTRIBUTE(CompoundShapeSettings::SubShapeSettings, mUserData)
}
...
void CompoundShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	outTotalVolume = 0.0f;
	...
	for (const SubShape &shape : mSubShapes)
	{
		...
		shape.mShape->GetSubmergedVolume(transform, shape.TransformScale(inScale), inSurface, total_volume, submerged_volume, center_of_buoyancy ACPH_IF_DEBUG_RENDERER(, inBaseOffset));
...
void CompoundShape::sCastCompoundVsShape(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	ACPH_ASSERT(inShapeCast.mShape->GetType() == EShapeType::Compound);
...
{
	ACPH_ASSERT(mSubShapes.size() == inNumShapes);
