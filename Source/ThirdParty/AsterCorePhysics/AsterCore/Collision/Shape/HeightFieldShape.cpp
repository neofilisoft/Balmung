...
#include <AsterCore/Physics/Collision/CollideSoftBodyVerticesVsTriangles.h>
...
//#define ACPH_DEBUG_HEIGHT_FIELD
...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(HeightFieldShapeSettings)
{
	ACPH_ADD_BASE_CLASS(HeightFieldShapeSettings, ShapeSettings)

	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mHeightSamples)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mOffset)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mScale)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mMinHeightValue)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mMaxHeightValue)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mMaterialsCapacity)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mSampleCount)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mBlockSize)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mBitsPerSample)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mMaterialIndices)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mMaterials)
	ACPH_ADD_ATTRIBUTE(HeightFieldShapeSettings, mActiveEdgeCosThresholdAngle)
}
	...
	mSampleCount(inSampleCount)
{
	...
	{
		ACPH_ASSERT(inMaterialList.empty());
		ACPH_ASSERT(inMaterialIndices == nullptr);
	}
}
...
void HeightFieldShape::CalculateActiveEdges(uint inX, uint inY, uint inSizeX, uint inSizeY, const float *inHeights, uint inHeightsStartX, uint inHeightsStartY, intptr_t inHeightsStride, float inHeightsScale, float inActiveEdgeCosThresholdAngle, TempAllocator &inAllocator)
{
	...
	ACPH_SCOPE_EXIT([&inAllocator, normals, normals_size]{ inAllocator.Free(normals, normals_size); });
		...
		for (uint block_x = 0; block_x < inSizeX; block_x += block_size_x)
		{
			...
			for (uint y = block_y; y < block_y_end; ++y)
			{
				...
				for (uint x = block_x; x < block_x_end; ++x)
				{
					...
					ACPH_ASSERT(byte_pos < mActiveEdgesSize);
	...
	if (mMaterials.size() > 1)
		for (uint y = 0; y < out_count_min_1; ++y)
			for (uint x = 0; x < out_count_min_1; ++x)
			{
				...
				ACPH_ASSERT(byte_pos + 1 < mMaterialIndices.size());
...
void HeightFieldShape::AllocateBuffers()
{
	...
	ACPH_ASSERT(mRangeBlocks == nullptr && mHeightSamples == nullptr && mActiveEdges == nullptr);
	...
	mBitsPerSample(uint8(inSettings.mBitsPerSample))
{
	...
	ACPH_ASSERT(cur_range_vector == &ranges.front());
	...
	ACPH_ASSERT(mMinSample == minv && mMaxSample == maxv);
#endif
	...
	{
		ACPH_ASSERT(uint(current_block - mRangeBlocks) == sGridOffsets[level]);
	...
	ACPH_ASSERT(uint32(current_block - mRangeBlocks) == mRangeBlocksSize);
	...
	for (uint y = 0; y < mSampleCount; ++y)
		for (uint x = 0; x < mSampleCount; ++x)
		{
			...
			else
			{
				...
				ACPH_ASSERT(range.mMin < range.mMax);
			...
			ACPH_ASSERT(byte_pos + 1 < mHeightSamplesSize);
...
{
	ACPH_ASSERT(inBlockX < GetNumBlocks() && inBlockY < GetNumBlocks());
	...
	ACPH_ASSERT(offset < mRangeBlocksSize);
...
{
	ACPH_ASSERT(inX < mSampleCount);
	ACPH_ASSERT(inY < mSampleCount);
...
void HeightFieldShape::GetHeights(uint inX, uint inY, uint inSizeX, uint inSizeY, float *outHeights, intptr_t inHeightsStride) const
{
	...
	ACPH_ASSERT(inX % mBlockSize == 0 && inY % mBlockSize == 0);
	ACPH_ASSERT(inX < mSampleCount && inY < mSampleCount);
	ACPH_ASSERT(inX + inSizeX <= mSampleCount && inY + inSizeY <= mSampleCount);
...
void HeightFieldShape::SetHeights(uint inX, uint inY, uint inSizeX, uint inSizeY, const float *inHeights, intptr_t inHeightsStride, TempAllocator &inAllocator, float inActiveEdgeCosThresholdAngle)
{
	...
	ACPH_ASSERT(mHeightSamplesSize > 0);
	ACPH_ASSERT(inX % mBlockSize == 0 && inY % mBlockSize == 0);
	ACPH_ASSERT(inX < mSampleCount && inY < mSampleCount);
	ACPH_ASSERT(inX + inSizeX <= mSampleCount && inY + inSizeY <= mSampleCount);
...
bool HeightFieldShape::SetMaterials(uint inX, uint inY, uint inSizeX, uint inSizeY, const uint8 *inMaterials, intptr_t inMaterialsStride, const PhysicsMaterialList *inMaterialList, TempAllocator &inAllocator)
{
	...
	ACPH_SCOPE_EXIT([&inAllocator, material_remap_table, material_remap_table_size]{ inAllocator.Free(material_remap_table, material_remap_table_size); });
	...
	ACPH_ASSERT(mNumBitsPerMaterialIndex <= 8 && new_bits_per_material_index <= 8);
...
const PhysicsMaterial *HeightFieldShape::GetMaterial(uint inX, uint inY) const
{
	...
	uint count_min_1 = mSampleCount - 1;
	ACPH_ASSERT(inX < count_min_1);
	ACPH_ASSERT(inY < count_min_1);
...
void HeightFieldShape::DecodeSubShapeID(const SubShapeID &inSubShapeID, uint &outX, uint &outY, uint &outTriangle) const
{
	...
	ACPH_ASSERT(remainder.IsEmpty(), "Invalid subshape ID");
...
{
	ACPH_ASSERT(inX < mSampleCount - 1 && inY < mSampleCount - 1);
	...
	if (inTriangle == 0)
	{
		...
		uint byte_pos = bit_pos >> 3;
		bit_pos &= 0b111;
		ACPH_ASSERT(byte_pos + 1 < mActiveEdgesSize);
...
void HeightFieldShape::Draw(DebugRenderer *inRenderer, RMat44Arg inCenterOfMassTransform, Vec3Arg inScale, ColorArg inColor, bool inUseMaterialColors, bool inDrawWireframe) const
{
	...
	if (sDrawTriangleOutlines)
	{
		struct Visitor
		{
			ACPH_INLINE explicit		Visitor(const HeightFieldShape *inShape, DebugRenderer *inRenderer, RMat44Arg inTransform) :
				mShape(inShape),
			...
			ACPH_INLINE bool			ShouldVisitRangeBlock([[maybe_unused]] int inStackTop) const
			{
				return true;
			}

			ACPH_INLINE int			VisitRangeBlock(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioProperties, [[maybe_unused]] int inStackTop) const
			{
				UVec4 valid = Vec4::sLessOrEqual(inBoundsMinY, inBoundsMaxY);
				return CountAndSortTrues(valid, ioProperties);
			}

			ACPH_INLINE void			VisitTriangle(uint inX, uint inY, uint inTriangle, Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2) const
			{
...
{
public:
	ACPH_INLINE explicit			DecodingContext(const HeightFieldShape *inShape) :
		mShape(inShape)
	{
	...
	ACPH_INLINE void				WalkHeightField(Visitor &ioVisitor)
	{
		...
		ACPH_IF_ENABLE_ASSERTS(UVec4 dummy = UVec4::sReplicate(0);)
		ACPH_ASSERT(ioVisitor.VisitRangeBlock(Vec4::sReplicate(-1.0e6f), Vec4::sReplicate(1.0e6f), Vec4::sReplicate(-1.0e6f), Vec4::sReplicate(1.0e6f), Vec4::sReplicate(-1.0e6f), Vec4::sReplicate(1.0e6f), dummy, 0) == 0);
		...
		Vec3 *vertices = reinterpret_cast<Vec3 *>(ACPH_STACK_ALLOC(array_size * sizeof(Vec3)));
		bool *no_collision = reinterpret_cast<bool *>(ACPH_STACK_ALLOC(array_size * sizeof(bool)));
		...
		do
		{
			...
			else
			{
				...
				ACPH_ASSERT(offset < mShape->mRangeBlocksSize);
	...
	ACPH_INLINE bool				IsDoneWalking() const
	{
		return mTop < 0;
	}
...
bool HeightFieldShape::CastRay(const RayCast &inRay, const SubShapeIDCreator &inSubShapeIDCreator, RayCastResult &ioHit) const
{
	...
	struct Visitor
	{
		ACPH_INLINE explicit		Visitor(const HeightFieldShape *inShape, const RayCast &inRay, const SubShapeIDCreator &inSubShapeIDCreator, RayCastResult &ioHit) :
			mHit(ioHit),
		...
		ACPH_INLINE bool			ShouldVisitRangeBlock(int inStackTop) const
		{
			return mDistanceStack[inStackTop] < mHit.mFraction;
		}

		ACPH_INLINE int			VisitRangeBlock(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioProperties, int inStackTop)
		{
		...
		ACPH_INLINE void			VisitTriangle(uint inX, uint inY, uint inTriangle, Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2)
		{
...
void HeightFieldShape::CastRay(const RayCast &inRay, const RayCastSettings &inRayCastSettings, const SubShapeIDCreator &inSubShapeIDCreator, CastRayCollector &ioCollector, const ShapeFilter &inShapeFilter) const
{
	...
	struct Visitor
	{
		ACPH_INLINE explicit		Visitor(const HeightFieldShape *inShape, const RayCast &inRay, const RayCastSettings &inRayCastSettings, const SubShapeIDCreator &inSubShapeIDCreator, CastRayCollector &ioCollector) :
...
void HeightFieldShape::CollideSoftBodyVertices(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const CollideSoftBodyVertexIterator &inVertices, uint inNumVertices, int inCollidingShapeIndex) const
{
	...
	struct Visitor : public CollideSoftBodyVerticesVsTriangles
	{
		...
		ACPH_INLINE void	VisitTriangle([[maybe_unused]] uint inX, [[maybe_unused]] uint inY, [[maybe_unused]] uint inTriangle, Vec3Arg inV0, Vec3Arg inV1, Vec3Arg inV2)
		{
...
void HeightFieldShape::sCastConvexVsHeightField(const ShapeCast &inShapeCast, const ShapeCastSettings &inShapeCastSettings, const Shape *inShape, Vec3Arg inScale, [[maybe_unused]] const ShapeFilter &inShapeFilter, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, CastShapeCollector &ioCollector)
{
	...
	ACPH_ASSERT(inShape->GetSubType() == EShapeSubType::HeightField);
...
void HeightFieldShape::GetTrianglesStart(GetTrianglesContext &ioContext, const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale) const
{
	...
	ACPH_ASSERT(IsAligned(&ioContext, alignof(HSGetTrianglesContext)));
...
int HeightFieldShape::GetTrianglesNext(GetTrianglesContext &ioContext, int inMaxTrianglesRequested, Float3 *outTriangleVertices, const PhysicsMaterial **outMaterials) const
{
	...
	ACPH_ASSERT(inMaxTrianglesRequested >= cGetTrianglesMinTrianglesRequested);
...
void HeightFieldShape::sCollideConvexVsHeightField(const Shape *inShape1, const Shape *inShape2, Vec3Arg inScale1, Vec3Arg inScale2, Mat44Arg inCenterOfMassTransform1, Mat44Arg inCenterOfMassTransform2, const SubShapeIDCreator &inSubShapeIDCreator1, const SubShapeIDCreator &inSubShapeIDCreator2, const CollideShapeSettings &inCollideShapeSettings, CollideShapeCollector &ioCollector, [[maybe_unused]] const ShapeFilter &inShapeFilter)
{
	...
	ACPH_ASSERT(inShape2->GetType() == EShapeType::HeightField);
