...
class ACPH_EXPORT BoxShapeSettings final : public ConvexShapeSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, BoxShapeSettings)
...
class ACPH_EXPORT BoxShape final : public ConvexShape
{
public:
	...
	/// Create a box with half edge length inHalfExtent and convex radius inConvexRadius.
	/// (internally the convex radius will be subtracted from the half extent so the total box will not grow with the convex radius).
	explicit				BoxShape(Vec3Arg inHalfExtent, float inConvexRadius = cDefaultConvexRadius, const PhysicsMaterial *inMaterial = nullptr) : ConvexShape(EShapeSubType::Box, inMaterial), mHalfExtent(inHalfExtent), mConvexRadius(min(inConvexRadius, inHalfExtent.ReduceMin())) { ACPH_ASSERT(inHalfExtent.ReduceMin() >= 0.0f); ACPH_ASSERT(inConvexRadius >= 0.0f); }
