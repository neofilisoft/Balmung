...
#include <AsterCore/Physics/Collision/Shape/PolyhedronSubmergedVolumeCalculator.h>
...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(ConvexHullShapeSettings)
{
	ACPH_ADD_BASE_CLASS(ConvexHullShapeSettings, ConvexShapeSettings)

	ACPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mPoints)
	ACPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mMaxConvexRadius)
	ACPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mMaxErrorConvexRadius)
	ACPH_ADD_ATTRIBUTE(ConvexHullShapeSettings, mHullTolerance)
}
	...
	mConvexRadius(inSettings.mMaxConvexRadius)
{
	...
	for (BuilderFace *builder_face : builder_faces)
	{
		...
		ACPH_ASSERT(mVertexIdx.size() <= 0xFFFF);
...
public:
		...
		mConvexRadius(inConvexRadius)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(HullNoConvex)));
	}
...
public:
		...
		mShape(inShape)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(HullWithConvex)));
	}
...
public:
		...
		mScale(inScale)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(HullWithConvexScaled)));
	}
...
const ConvexShape::Support *ConvexHullShape::GetSupportFunction(ESupportMode inMode, SupportBuffer &inBuffer, Vec3Arg inScale) const
{
	...
	case ESupportMode::ExcludeConvexRadius:
		if (ScaleHelpers::IsNotScaled(inScale))
		{
			...
			ACPH_ASSERT(mPoints.size() <= cMaxPointsInHull, "Not enough space, this should have been caught during shape creation!");
...
void ConvexHullShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	...
	PolyhedronSubmergedVolumeCalculator::Point *buffer = (PolyhedronSubmergedVolumeCalculator::Point *)ACPH_STACK_ALLOC(num_points * sizeof(PolyhedronSubmergedVolumeCalculator::Point));
	PolyhedronSubmergedVolumeCalculator submerged_vol_calc(inCenterOfMassTransform * Mat44::sScale(inScale), &mPoints[0].mPosition, sizeof(Point), num_points, inSurface, buffer ACPH_IF_DEBUG_RENDERER(, inBaseOffset));
...
void ConvexHullShape::GetTrianglesStart(GetTrianglesContext &ioContext, const AABox &inBox, Vec3Arg inPositionCOM, QuatArg inRotation, Vec3Arg inScale) const
{
	...
	ACPH_ASSERT(IsAligned(&ioContext, alignof(CHSGetTrianglesContext)));
