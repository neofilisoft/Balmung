...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(ScaledShapeSettings)
{
	ACPH_ADD_BASE_CLASS(ScaledShapeSettings, DecoratedShapeSettings)

	ACPH_ADD_ATTRIBUTE(ScaledShapeSettings, mScale)
}
...
void ScaledShape::GetSubmergedVolume(Mat44Arg inCenterOfMassTransform, Vec3Arg inScale, const Plane &inSurface, float &outTotalVolume, float &outSubmergedVolume, Vec3 &outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, RVec3Arg inBaseOffset)) const
{
	mInnerShape->GetSubmergedVolume(inCenterOfMassTransform, inScale * mScale, inSurface, outTotalVolume, outSubmergedVolume, outCenterOfBuoyancy ACPH_IF_DEBUG_RENDERER(, inBaseOffset));
}
...
{
	ACPH_ASSERT(inShape1->GetSubType() == EShapeSubType::Scaled);
...
{
	ACPH_ASSERT(inShape2->GetSubType() == EShapeSubType::Scaled);
...
{
	ACPH_ASSERT(inShapeCast.mShape->GetSubType() == EShapeSubType::Scaled);
...
{
	ACPH_ASSERT(inShape->GetSubType() == EShapeSubType::Scaled);
