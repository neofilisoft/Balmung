...
class ACPH_EXPORT SphereShapeSettings final : public ConvexShapeSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, SphereShapeSettings)
...
class ACPH_EXPORT SphereShape final : public ConvexShape
{
public:
	...
	/// Create a sphere with radius inRadius
	explicit				SphereShape(float inRadius, const PhysicsMaterial *inMaterial = nullptr)			: ConvexShape(EShapeSubType::Sphere, inMaterial), mRadius(inRadius) { ACPH_ASSERT(inRadius > 0.0f); }
	...
	// See Shape::GetVolume
	virtual float			GetVolume() const override															{ return 4.0f / 3.0f * ACPH_PI * Cubed(mRadius); }
