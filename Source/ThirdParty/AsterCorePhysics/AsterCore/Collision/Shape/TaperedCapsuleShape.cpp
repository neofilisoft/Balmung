...
ACPH_IMPLEMENT_SERIALIZABLE_VIRTUAL(TaperedCapsuleShapeSettings)
{
	ACPH_ADD_BASE_CLASS(TaperedCapsuleShapeSettings, ConvexShapeSettings)

	ACPH_ADD_ATTRIBUTE(TaperedCapsuleShapeSettings, mHalfHeightOfTaperedCylinder)
	ACPH_ADD_ATTRIBUTE(TaperedCapsuleShapeSettings, mTopRadius)
	ACPH_ADD_ATTRIBUTE(TaperedCapsuleShapeSettings, mBottomRadius)
}
	...
	mBottomRadius(inSettings.mBottomRadius)
{
	...
	ACPH_ASSERT(mConvexRadius > 0.0f);
	...
	ACPH_ASSERT(mSinAlpha >= -1.0f && mSinAlpha <= 1.0f);
...
public:
		...
		mConvexRadius(inConvexRadius)
	{
		...
		ACPH_ASSERT(IsAligned(this, alignof(TaperedCapsule)));
	}
	...
	case ESupportMode::Default:
		{
			...
			ACPH_ASSERT(tr >= 0.0f && br >= 0.0f);
			ACPH_ASSERT(tr == 0.0f || br == 0.0f, "Convex radius should be that of the smallest sphere");
