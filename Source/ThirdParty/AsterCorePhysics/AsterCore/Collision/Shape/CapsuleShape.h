...
class ACPH_EXPORT CapsuleShapeSettings final : public ConvexShapeSettings
{
	ACPH_DECLARE_SERIALIZABLE_VIRTUAL(ACPH_EXPORT, CapsuleShapeSettings)
...
class ACPH_EXPORT CapsuleShape final : public ConvexShape
{
public:
	...
	/// Create a capsule centered around the origin with one sphere cap at (0, -inHalfHeightOfCylinder, 0) and the other at (0, inHalfHeightOfCylinder, 0)
							CapsuleShape(float inHalfHeightOfCylinder, float inRadius, const PhysicsMaterial *inMaterial = nullptr) : ConvexShape(EShapeSubType::Capsule, inMaterial), mRadius(inRadius), mHalfHeightOfCylinder(inHalfHeightOfCylinder) { ACPH_ASSERT(inHalfHeightOfCylinder > 0.0f); ACPH_ASSERT(inRadius > 0.0f); }
	...
	// See Shape::GetVolume
	virtual float			GetVolume() const override												{ return 4.0f / 3.0f * ACPH_PI * Cubed(mRadius) + 2.0f * ACPH_PI * mHalfHeightOfCylinder * Square(mRadius); }
