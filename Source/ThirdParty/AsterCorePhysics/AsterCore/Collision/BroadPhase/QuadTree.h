...
//#define ACPH_DUMP_BROADPHASE_TREE
...
class ACPH_EXPORT QuadTree : public NonCopyable
{
public:
...
private:
	...
	public:
		...
		static inline NodeID	sFromBodyID(BodyID inID)			{ NodeID node_id(inID.GetIndexAndSequenceNumber()); ACPH_ASSERT(node_id.IsBody()); return node_id; }
		static inline NodeID	sFromNodeIndex(uint32 inIdx)		{ ACPH_ASSERT((inIdx & cIsNode) == 0); return NodeID(inIdx | cIsNode); }
		...
		inline BodyID			GetBodyID() const					{ ACPH_ASSERT(IsBody()); return BodyID(mID); }
		inline uint32			GetNodeIndex() const				{ ACPH_ASSERT(IsNode()); return mID & ~cIsNode; }
...
private:
	...
	ACPH_INLINE const RootNode &	GetCurrentRoot() const				{ return mRootNode[mRootNodeIndex]; }
	ACPH_INLINE RootNode &		GetCurrentRoot()					{ return mRootNode[mRootNodeIndex]; }
	...
	/// when reading nodes.
	alignas(ACPH_CACHE_LINE_SIZE) atomic<uint32> mNumBodies { 0 };
	...
	ACPH_INLINE void				WalkTree(const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking, Visitor &ioVisitor ACPH_IF_TRACK_BROADPHASE_STATS(, LayerToStats &ioStats)) const;
