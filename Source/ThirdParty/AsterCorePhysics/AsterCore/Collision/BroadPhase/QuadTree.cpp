...
#include <AsterCore/Physics/Collision/BroadPhase/QuadTree.h>
...
void QuadTree::Node::SetChildBounds(int inChildIndex, const AABox &inBounds)
{
	...
	ACPH_ASSERT(inBounds.mMin.GetX() >= -cLargeFloat && inBounds.mMin.GetX() <= cLargeFloat
...
void QuadTree::GetBodyLocation(const TrackingVector &inTracking, BodyID inBodyID, uint32 &outNodeIdx, uint32 &outChildIdx) const
{
	...
	ACPH_ASSERT(body_location != Tracking::cInvalidBodyLocation);
	outNodeIdx = body_location & 0x3fffffff;
	outChildIdx = body_location >> 30;
	ACPH_ASSERT(mAllocator->Get(outNodeIdx).mChildNodeID[outChildIdx] == inBodyID, "Make sure that the body is in the node where it should be");
}

void QuadTree::SetBodyLocation(TrackingVector &ioTracking, BodyID inBodyID, uint32 inNodeIdx, uint32 inChildIdx) const
{
	ACPH_ASSERT(inNodeIdx <= 0x3fffffff);
	ACPH_ASSERT(inChildIdx < 4);
	ACPH_ASSERT(mAllocator->Get(inNodeIdx).mChildNodeID[inChildIdx] == inBodyID, "Make sure that the body is in the node where it should be");
	ioTracking[inBodyID.GetIndex()].mBodyLocation = inNodeIdx + (inChildIdx << 30);

#ifdef ACPH_ENABLE_ASSERTS
	uint32 v1, v2;
	GetBodyLocation(ioTracking, inBodyID, v1, v2);
	ACPH_ASSERT(v1 == inNodeIdx);
	ACPH_ASSERT(v2 == inChildIdx);
#endif
}
...
QuadTree::~QuadTree()
{
	...
	ACPH_ASSERT(node_stack.front().IsValid());
...
uint32 QuadTree::AllocateNode(bool inIsChanged)
{
	...
	if (index == Allocator::cInvalidObjectIndex)
	{
		...
		//
		// If you have created a wrapper around AsterCore then a possible solution is to activate a mode during loading
...
AABox QuadTree::GetBounds() const
{
	...
	ACPH_ASSERT(node_idx != cInvalidNodeIndex);
	const Node &node = mAllocator->Get(node_idx);
...
void QuadTree::UpdatePrepare(const BodyVector &inBodies, TrackingVector &ioTracking, UpdateState &outUpdateState, bool inFullRebuild)
{
	...
	ACPH_ASSERT(mFreeNodeBatch.mNumObjects == 0);
	...
	do
	{
		...
		if (node_id.IsBody())
		{
			...
			ACPH_ASSERT(ioTracking[body_index].mObjectLayer == inBodies[body_index]->GetObjectLayer());
		#endif
	...
	ACPH_ASSERT(inFullRebuild? num_node_ids == mNumBodies : num_node_ids <= mNumBodies);
...
void QuadTree::UpdateFinalize([[maybe_unused]] const BodyVector &inBodies, [[maybe_unused]] const TrackingVector &inTracking, const UpdateState &inUpdateState)
{
	...
	RootNode &new_root_node = mRootNode[new_root_idx];
	{
		...
		ACPH_ASSERT(new_root_node.mIndex == cInvalidNodeIndex);
...
QuadTree::NodeID QuadTree::BuildTree(const BodyVector &inBodies, TrackingVector &ioTracking, NodeID *ioNodeIDs, int inNumber, uint inMaxDepthMarkChanged, AABox &outBounds)
{
	...
	ACPH_ASSERT(IsAligned(centers, ACPH_VECTOR_ALIGNMENT));
	Vec3 *c = centers;
...
void QuadTree::WidenAndMarkNodeAndParentsChanged(uint32 inNodeIndex, const AABox &inNewBounds)
{
	...
	for (;;)
	{
		...
		ACPH_ASSERT(child_idx != -1, "Nodes don't get removed from the tree, we must have found it");
...
void QuadTree::AddBodiesAbort(TrackingVector &ioTracking, const AddState &inState)
{
	...
	ACPH_ASSERT(node_stack[0].IsValid());
	int top = 0;
	do
	{
		// Check if node is a body
...
void QuadTree::RemoveBodies([[maybe_unused]] const BodyVector &inBodies, TrackingVector &ioTracking, const BodyID *ioBodyIDs, int inNumber)
{
	...
	for (const BodyID *cur = ioBodyIDs, *end = ioBodyIDs + inNumber; cur < end; ++cur)
	{
		...
		ACPH_ASSERT(inBodies[cur->GetIndex()]->GetID() == *cur, "Provided BodyID doesn't match BodyID in body manager");
...
void QuadTree::NotifyBodiesAABBChanged(const BodyVector &inBodies, const TrackingVector &inTracking, const BodyID *ioBodyIDs, int inNumber)
{
	...
	for (const BodyID *cur = ioBodyIDs, *end = ioBodyIDs + inNumber; cur < end; ++cur)
	{
		...
		ACPH_ASSERT(body->GetID() == *cur, "Provided BodyID doesn't match BodyID in body manager");
...
ACPH_INLINE void QuadTree::WalkTree(const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking, Visitor &ioVisitor ACPH_IF_TRACK_BROADPHASE_STATS(, LayerToStats &ioStats)) const
{
	// Get the root
	const RootNode &root_node = GetCurrentRoot();
	...
	do
	{
		...
		if (child_node_id.IsBody())
		{
			...
			ACPH_IF_TRACK_BROADPHASE_STATS(++bodies_visited;)
			...
			{
				ACPH_PROFILE("VisitBody");

				// Track amount of hits
				ACPH_IF_TRACK_BROADPHASE_STATS(++hits_collected;)

				// Start track time the collector takes
				ACPH_IF_TRACK_BROADPHASE_STATS(uint64 collector_start = GetProcessorTickCount();)

				// We found a body we collide with, call our visitor
				ioVisitor.VisitBody(body_id, top);

				// End track time the collector takes
				ACPH_IF_TRACK_BROADPHASE_STATS(collector_ticks += GetProcessorTickCount() - collector_start;)
		...
		{
			ACPH_IF_TRACK_BROADPHASE_STATS(++nodes_visited;)
			...
			ACPH_ASSERT(IsAligned(&node, ACPH_CACHE_LINE_SIZE));
...
void QuadTree::CastRay(const RayCast &inRay, RayCastBodyCollector &ioCollector, const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking) const
{
	...
	public:
		/// Constructor
		ACPH_INLINE				Visitor(const RayCast &inRay, RayCastBodyCollector &ioCollector) :
		...
		ACPH_INLINE bool			ShouldAbort() const
		{
			return mCollector.ShouldEarlyOut();
		}

		/// Returns true if this node / body should be visited, false if no hit can be generated
		ACPH_INLINE bool			ShouldVisitNode(int inStackTop) const
		{
		...
		ACPH_INLINE int			VisitNodes(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioChildNodeIDs, int inStackTop)
		{
		...
		ACPH_INLINE void			VisitBody(const BodyID &inBodyID, int inStackTop)
		{
		...
		ACPH_INLINE void			OnStackResized(size_t inNewStackSize)
		{
	...
	WalkTree(inObjectLayerFilter, inTracking, visitor ACPH_IF_TRACK_BROADPHASE_STATS(, mCastRayStats));
}
...
void QuadTree::CollideAABox(const AABox &inBox, CollideShapeBodyCollector &ioCollector, const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking) const
{
	...
	public:
		/// Constructor
		ACPH_INLINE					Visitor(const AABox &inBox, CollideShapeBodyCollector &ioCollector) :
			mBox(inBox),
			mCollector(ioCollector)
		{
		}
		...
		/// Visit nodes, returns number of hits found and sorts ioChildNodeIDs so that they are at the beginning of the vector.
		ACPH_INLINE int				VisitNodes(Vec4Arg inBoundsMinX, Vec4Arg inBoundsMinY, Vec4Arg inBoundsMinZ, Vec4Arg inBoundsMaxX, Vec4Arg inBoundsMaxY, Vec4Arg inBoundsMaxZ, UVec4 &ioChildNodeIDs, int inStackTop) const
		{
		...
		ACPH_INLINE void				OnStackResized([[maybe_unused]] size_t inNewStackSize) const
		{
			// Nothing to do
		}
	...
	WalkTree(inObjectLayerFilter, inTracking, visitor ACPH_IF_TRACK_BROADPHASE_STATS(, mCollideAABoxStats));
}
...
void QuadTree::CollideSphere(Vec3Arg inCenter, float inRadius, CollideShapeBodyCollector &ioCollector, const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking) const
{
	...
	public:
		/// Constructor
		ACPH_INLINE					Visitor(Vec3Arg inCenter, float inRadius, CollideShapeBodyCollector &ioCollector) :
...
private:
	...
	WalkTree(inObjectLayerFilter, inTracking, visitor ACPH_IF_TRACK_BROADPHASE_STATS(, mCollideSphereStats));
}
...
void QuadTree::CollidePoint(Vec3Arg inPoint, CollideShapeBodyCollector &ioCollector, const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking) const
{
	...
	public:
		/// Constructor
		ACPH_INLINE					Visitor(Vec3Arg inPoint, CollideShapeBodyCollector &ioCollector) :
			mPoint(inPoint),
			mCollector(ioCollector)
		{
		}
	...
	WalkTree(inObjectLayerFilter, inTracking, visitor ACPH_IF_TRACK_BROADPHASE_STATS(, mCollidePointStats));
}
...
void QuadTree::CollideOrientedBox(const OrientedBox &inBox, CollideShapeBodyCollector &ioCollector, const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking) const
{
	...
	public:
		/// Constructor
		ACPH_INLINE					Visitor(const OrientedBox &inBox, CollideShapeBodyCollector &ioCollector) :
			mBox(inBox),
			mCollector(ioCollector)
		{
		}
	...
	Visitor visitor(inBox, ioCollector);
	WalkTree(inObjectLayerFilter, inTracking, visitor ACPH_IF_TRACK_BROADPHASE_STATS(, mCollideOrientedBoxStats));
}
...
void QuadTree::CastAABox(const AABoxCast &inBox, CastShapeBodyCollector &ioCollector, const ObjectLayerFilter &inObjectLayerFilter, const TrackingVector &inTracking) const
{
	...
	public:
		/// Constructor
		ACPH_INLINE					Visitor(const AABoxCast &inBox, CastShapeBodyCollector &ioCollector) :
	...
	Visitor visitor(inBox, ioCollector);
	WalkTree(inObjectLayerFilter, inTracking, visitor ACPH_IF_TRACK_BROADPHASE_STATS(, mCastAABoxStats));
}
...
void QuadTree::FindCollidingPairs(const BodyVector &inBodies, const BodyID *inActiveBodies, int inNumActiveBodies, float inSpeculativeContactDistance, BodyPairCollector &ioPairCollector, const ObjectLayerPairFilter &inObjectLayerPairFilter) const
{
	...
	const RootNode &root_node = GetCurrentRoot();
	ACPH_ASSERT(root_node.mIndex != cInvalidNodeIndex);

	// Assert sane input
	ACPH_ASSERT(inActiveBodies != nullptr);
	ACPH_ASSERT(inNumActiveBodies > 0);
	...
	for (int b1 = 0; b1 < inNumActiveBodies; ++b1)
	{
		...
		ACPH_ASSERT(!body1.IsStatic());

	#ifdef ACPH_TRACK_SIMULATION_STATS
	...
	ACPH_ASSERT(root_node.mIndex != cInvalidNodeIndex);
	ACPH_ASSERT(&root_node == &GetCurrentRoot());
}
...
void QuadTree::ValidateTree(const BodyVector &inBodies, const TrackingVector &inTracking, uint32 inNodeIndex, uint32 inNumExpectedBodies) const
{
	...
	ACPH_ASSERT(inNodeIndex != cInvalidNodeIndex);

	// To avoid call overhead, create a stack in place
	ACPH_SUPPRESS_WARNING_PUSH
	ACPH_CLANG_SUPPRESS_WARNING("-Wunused-member-function") // The default constructor of StackEntry is unused when using AsterCore's Array class but not when using std::vector
	struct StackEntry
	{
						StackEntry() = default;
	...
	do
	{
		...
		ACPH_ASSERT(node.mParentNodeIndex == cur_stack.mParentNodeIndex);

		// Validate that when a parent is not-changed that all of its children are also
		ACPH_ASSERT(cur_stack.mParentNodeIndex == cInvalidNodeIndex || mAllocator->Get(cur_stack.mParentNodeIndex).mIsChanged || !node.mIsChanged);
		...
		for (uint32 i = 0; i < 4; ++i)
		{
			...
			if (child_node_id.IsValid())
			{
				if (child_node_id.IsNode())
				{
					...
					ACPH_ASSERT(child_bounds.Contains(real_child_bounds) || !real_child_bounds.IsValid());
				}
				else
				{
					...
					ACPH_ASSERT(node_idx == cur_stack.mNodeIndex);
					ACPH_ASSERT(child_idx == i);
					...
					ACPH_ASSERT(body_bounds.Contains(body->GetWorldSpaceBounds()));
				}
			}
		}
	}
	...
	ACPH_ASSERT(num_bodies == inNumExpectedBodies);
}
