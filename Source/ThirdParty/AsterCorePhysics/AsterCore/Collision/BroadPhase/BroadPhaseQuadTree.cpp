...
void BroadPhaseQuadTree::Init(BodyManager *inBodyManager, const BroadPhaseLayerInterface &inLayerInterface)
{
	...
	ACPH_ASSERT(mNumLayers < (BroadPhaseLayer::Type)cBroadPhaseLayerInvalid);
...
void BroadPhaseQuadTree::FrameSync()
{
	...
	UniqueLock root_lock(mQueryLocks[mQueryLockIdx ^ 1] ACPH_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseQuery));
...
void BroadPhaseQuadTree::LockModifications()
{
	...
	PhysicsLock::sLock(mUpdateMutex ACPH_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
}

BroadPhase::UpdateState BroadPhaseQuadTree::UpdatePrepare()
{
	// LockModifications should have been called
	ACPH_ASSERT(mUpdateMutex.is_locked());
...
void BroadPhaseQuadTree::UnlockModifications()
{
	...
	PhysicsLock::sUnlock(mUpdateMutex ACPH_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
}
...
BroadPhase::AddState BroadPhaseQuadTree::AddBodiesPrepare(BodyID *ioBodies, int inNumber)
{
	...
	ACPH_ASSERT(mMaxBodies == mBodyManager->GetMaxBodies());
	...
	while (b_start < b_end)
	{
		...
		ACPH_ASSERT(broadphase_layer < mNumLayers);
		...
		for (const BodyID *b = b_start; b < b_mid; ++b)
		{
			...
			ACPH_ASSERT(bodies[index]->GetID() == *b, "Provided BodyID doesn't match BodyID in body manager");
			ACPH_ASSERT(!bodies[index]->IsInBroadPhase());
			Tracking &t = mTracking[index];
			ACPH_ASSERT(t.mBroadPhaseLayer == (BroadPhaseLayer::Type)cBroadPhaseLayerInvalid);
			t.mBroadPhaseLayer = broadphase_layer;
			ACPH_ASSERT(t.mObjectLayer == cObjectLayerInvalid);
...
void BroadPhaseQuadTree::AddBodiesFinalize(BodyID *ioBodies, int inNumber, AddState inAddState)
{
	...
	if (inNumber <= 0)
	{
		ACPH_ASSERT(inAddState == nullptr);
		return;
	}

	// This cannot run concurrently with UpdatePrepare()/UpdateFinalize()
	SharedLock lock(mUpdateMutex ACPH_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
	...
	for (BroadPhaseLayer::Type broadphase_layer = 0; broadphase_layer < mNumLayers; broadphase_layer++)
	{
		...
		if (l.mBodyStart != nullptr)
		{
			...
			for (const BodyID *b = l.mBodyStart; b < l.mBodyEnd; ++b)
			{
				uint32 index = b->GetIndex();
				ACPH_ASSERT(bodies[index]->GetID() == *b, "Provided BodyID doesn't match BodyID in body manager");
				ACPH_ASSERT(mTracking[index].mBroadPhaseLayer == broadphase_layer);
				ACPH_ASSERT(mTracking[index].mObjectLayer == bodies[index]->GetObjectLayer());
				ACPH_ASSERT(!bodies[index]->IsInBroadPhase());
...
void BroadPhaseQuadTree::AddBodiesAbort(BodyID *ioBodies, int inNumber, AddState inAddState)
{
	...
	ACPH_IF_ENABLE_ASSERTS(const BodyVector &bodies = mBodyManager->GetBodies();)
	ACPH_ASSERT(mMaxBodies == mBodyManager->GetMaxBodies());
	...
	for (BroadPhaseLayer::Type broadphase_layer = 0; broadphase_layer < mNumLayers; broadphase_layer++)
	{
		...
		if (l.mBodyStart != nullptr)
		{
			...
			for (const BodyID *b = l.mBodyStart; b < l.mBodyEnd; ++b)
			{
				...
				ACPH_ASSERT(!bodies[index]->IsInBroadPhase());
				Tracking &t = mTracking[index];
				ACPH_ASSERT(t.mBroadPhaseLayer == broadphase_layer);
...
void BroadPhaseQuadTree::RemoveBodies(BodyID *ioBodies, int inNumber)
{
	...
	while (b_start < b_end)
	{
		...
		ACPH_ASSERT(broadphase_layer != (BroadPhaseLayer::Type)cBroadPhaseLayerInvalid);
...
void BroadPhaseQuadTree::NotifyBodiesAABBChanged(BodyID *ioBodies, int inNumber, bool inTakeLock)
{
	...
	// This cannot run concurrently with UpdatePrepare()/UpdateFinalize()
	if (inTakeLock)
		PhysicsLock::sLockShared(mUpdateMutex ACPH_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
	else
		ACPH_ASSERT(mUpdateMutex.is_locked());
	...
	if (inTakeLock)
		PhysicsLock::sUnlockShared(mUpdateMutex ACPH_IF_ENABLE_ASSERTS(, mLockContext, EPhysicsLockTypes::BroadPhaseUpdate));
}
...
void BroadPhaseQuadTree::NotifyBodiesLayerChanged(BodyID *ioBodies, int inNumber)
{
	...
	for (BodyID *body_id = ioBodies + inNumber - 1; body_id >= ioBodies; --body_id)
	{
		...
		ACPH_ASSERT(bodies[index]->GetID() == *body_id, "Provided BodyID doesn't match BodyID in body manager");
...
void BroadPhaseQuadTree::FindCollidingPairs(BodyID *ioActiveBodies, int inNumActiveBodies, float inSpeculativeContactDistance, const ObjectVsBroadPhaseLayerFilter &inObjectVsBroadPhaseLayerFilter, const ObjectLayerPairFilter &inObjectLayerPairFilter, BodyPairCollector &ioPairCollector) const
{
	...
	while (b_start < b_end)
	{
		...
		ACPH_ASSERT(object_layer != cObjectLayerInvalid);
