...
bool EigenValueSymmetric(const Matrix &inMatrix, Matrix &outEigVec, Vector &outEigVal)
{
	...
	ACPH_ASSERT(inMatrix.GetRows() == n);
	ACPH_ASSERT(inMatrix.GetCols() == n);
	ACPH_ASSERT(outEigVec.GetRows() == n);
	ACPH_ASSERT(outEigVec.GetCols() == n);
	ACPH_ASSERT(outEigVal.GetRows() == n);
	ACPH_ASSERT(outEigVec.IsIdentity());
		...
		if (avg_sm < FLT_MIN) // Original code: sm == 0.0f, when the average is denormal, we also consider it machine underflow
		{
			// Sanity checks
				...
				for (uint c = 0; c < n; ++c)
				{
					...
					ACPH_ASSERT(outEigVec.GetColumn(c).IsNormalized());
					...
					ACPH_ASSERT(mat_eigvec.IsClose(eigval_eigvec, max(mat_eigvec.LengthSq(), eigval_eigvec.LengthSq()) * 1.0e-6f));
				}
			#endif
		...
		for (uint ip = 0; ip < n - 1; ++ip)
			for (uint iq = ip + 1; iq < n; ++iq)
			{
				...
				else if (abs_a_pq > thresh)
				{
					...
					#define ACPH_EVS_ROTATE(a, i, j, k, l)		\
						g = a(i, j),							\
						h = a(k, l),							\
						a(i, j) = g - s * (h + g * tau),		\
						a(k, l) = h + s * (g - h * tau)
					...
					uint j;
					for (j = 0; j < ip; ++j)		ACPH_EVS_ROTATE(a, j, ip, j, iq);
					for (j = ip + 1; j < iq; ++j)	ACPH_EVS_ROTATE(a, ip, j, j, iq);
					for (j = iq + 1; j < n; ++j)	ACPH_EVS_ROTATE(a, ip, j, iq, j);
					for (j = 0; j < n; ++j)			ACPH_EVS_ROTATE(outEigVec, j, ip, j, iq);

					#undef ACPH_EVS_ROTATE
				}
			}
	...
	// Failure
	ACPH_ASSERT(false, "Too many iterations");
	return false;
}
