...
#elif defined(ACPH_USE_NEON)
	return ACPH_NEON_SHUFFLE_F32x4(mValue, mValue, SwizzleX, SwizzleY, SwizzleZ, SwizzleW);
#else
...
ACPH_INLINE Vec4 Vec4::FlipSign() const
{
...
#elif defined(ACPH_USE_NEON)
	int32x4_t shift = ACPH_NEON_INT32x4(0, 1, 2, 3);
...
Vec4 Vec4::Tan() const
{
	...
	UVec4 bit1 = quadrant.LogicalShiftLeft<31>();
	tan = Vec4::sSelect(tan, Vec4::sReplicate(-1.0f) / (tan ACPH_IF_FLOATING_POINT_EXCEPTIONS_ENABLED(+ Vec4::sReplicate(FLT_MIN))), bit1); // Add small epsilon to prevent div by zero, works because tan is always positive
...
Vec4 Vec4::ASin() const
{
	...
	z = Vec4::sSelect(z, Vec4::sReplicate(0.5f * ACPH_PI) - (z + z), greater);
...
Vec4 Vec4::ACos() const
{
	...
	return Vec4::sReplicate(0.5f * ACPH_PI) - ASin();
}
...
Vec4 Vec4::ATan() const
{
	...
	Vec4 x2 = Vec4::sReplicate(-1.0f) / (x ACPH_IF_FLOATING_POINT_EXCEPTIONS_ENABLED(+ Vec4::sReplicate(FLT_MIN))); // Add small epsilon to prevent div by zero, works because x is always positive

	// Apply first if
	x = Vec4::sSelect(x, x1, greater1);
	y = Vec4::sSelect(y, Vec4::sReplicate(0.25f * ACPH_PI), greater1);

	// Apply second if
	x = Vec4::sSelect(x, x2, greater2);
	y = Vec4::sSelect(y, Vec4::sReplicate(0.5f * ACPH_PI), greater2);
...
Vec4 Vec4::sATan2(Vec4Arg inY, Vec4Arg inX)
{
	...
	atan = Vec4::sSelect(atan, Vec4::sReplicate(0.5f * ACPH_PI) - atan, x_is_numerator);
	...
	atan -= Vec4::sAnd(x_sign.ArithmeticShiftRight<31>().ReinterpretAsFloat(), Vec4::sReplicate(ACPH_PI));
