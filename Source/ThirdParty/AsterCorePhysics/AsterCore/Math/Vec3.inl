...
#include <AsterCore/Math/UVec4.h>
...
// Create a std::hash/ACPH::Hash for Vec3
ACPH_MAKE_HASHABLE(ACPH::Vec3, t.GetX(), t.GetY(), t.GetZ())
...
void Vec3::CheckW() const
{
	...
	ACPH_ASSERT(reinterpret_cast<const uint32 *>(mF32)[2] == reinterpret_cast<const uint32 *>(mF32)[3]);
#endif // ACPH_FLOATING_POINT_EXCEPTIONS_ENABLED
}

ACPH_INLINE Vec3::Type Vec3::sFixW(Type inValue)
{
#ifdef ACPH_FLOATING_POINT_EXCEPTIONS_ENABLED
	#if defined(ACPH_USE_SSE)
		return _mm_shuffle_ps(inValue, inValue, _MM_SHUFFLE(2, 2, 1, 0));
	#elif defined(ACPH_USE_NEON)
		return ACPH_NEON_SHUFFLE_F32x4(inValue, inValue, 0, 1, 2, 2);
	#else
		Type value;
...
Vec3::Vec3(const Float3 &inV)
{
	...
	mF32[1] = inV[1];
	mF32[2] = inV[2];
	mF32[3] = inV[2]; // Not strictly needed when ACPH_FLOATING_POINT_EXCEPTIONS_ENABLED is off but prevents warnings about uninitialized variables
#endif
}
...
Vec3::Vec3(float inX, float inY, float inZ)
{
	...
	mF32[1] = inY;
	mF32[2] = inZ;
	mF32[3] = inZ; // Not strictly needed when ACPH_FLOATING_POINT_EXCEPTIONS_ENABLED is off but prevents warnings about uninitialized variables
#endif
}
...
#elif defined(ACPH_USE_NEON)
	return ACPH_NEON_SHUFFLE_F32x4(mValue, mValue, SwizzleX, SwizzleY, SwizzleZ, SwizzleZ);
#else
...
{
#if defined(ACPH_USE_SSE4_1) && !defined(ACPH_PLATFORM_WASM) // _mm_blendv_ps has problems on FireFox
...
Vec3 Vec3::sRandom(Random &inRandom)
{
	...
	float theta = ACPH_PI * zero_to_one(inRandom);
	float phi = 2.0f * ACPH_PI * zero_to_one(inRandom);
...
#elif defined(ACPH_USE_NEON)
	Type t1 = ACPH_NEON_SHUFFLE_F32x4(inV2.mValue, inV2.mValue, 1, 2, 0, 0); // Assure Z and W are the same
	t1 = vmulq_f32(t1, mValue);
	Type t2 = ACPH_NEON_SHUFFLE_F32x4(mValue, mValue, 1, 2, 0, 0); // Assure Z and W are the same
	t2 = vmulq_f32(t2, inV2.mValue);
	Type t3 = vsubq_f32(t1, t2);
	return ACPH_NEON_SHUFFLE_F32x4(t3, t3, 1, 2, 0, 0); // Assure Z and W are the same
#else
...
#elif defined(ACPH_USE_NEON)
	uint32x4_t mask = ACPH_NEON_UINT32x4(1, 1, 1, 0);
...
ACPH_INLINE Vec3 Vec3::FlipSign() const
{
