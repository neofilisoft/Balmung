...
class [[nodiscard]] alignas(ACPH_VECTOR_ALIGNMENT) Quat
{
public:
	...
	ACPH_INLINE float			GetX() const													{ return mValue.GetX(); }

	/// Get Y component (imaginary part j)
	ACPH_INLINE float			GetY() const													{ return mValue.GetY(); }

	/// Get Z component (imaginary part k)
	ACPH_INLINE float			GetZ() const													{ return mValue.GetZ(); }

	/// Get W component (real part)
	ACPH_INLINE float			GetW() const													{ return mValue.GetW(); }

	/// Get the imaginary part of the quaternion
	ACPH_INLINE Vec3				GetXYZ() const													{ return Vec3(mValue); }

	/// Get the quaternion as a Vec4
	ACPH_INLINE Vec4				GetXYZW() const													{ return mValue; }

	/// Set individual components
	ACPH_INLINE void				SetX(float inX)													{ mValue.SetX(inX); }
	ACPH_INLINE void				SetY(float inY)													{ mValue.SetY(inY); }
	ACPH_INLINE void				SetZ(float inZ)													{ mValue.SetZ(inZ); }
	ACPH_INLINE void				SetW(float inW)													{ mValue.SetW(inW); }

	/// Set all components
	ACPH_INLINE void				Set(float inX, float inY, float inZ, float inW)					{ mValue.Set(inX, inY, inZ, inW); }
	...
	/// @return [0, 0, 0, 0]
	ACPH_INLINE static Quat		sZero()															{ return Quat(Vec4::sZero()); }

	/// @return [1, 0, 0, 0] (or in storage format Quat(0, 0, 0, 1))
	ACPH_INLINE static Quat		sIdentity()														{ return Quat(0, 0, 0, 1); }

	///@}

	/// Rotation from axis and angle
	ACPH_INLINE static Quat		sRotation(Vec3Arg inAxis, float inAngle);

	/// Get axis and angle that represents this quaternion, outAngle will always be in the range \f$[0, \pi]\f$
	ACPH_INLINE void				GetAxisAngle(Vec3 &outAxis, float &outAngle) const;
	...
	ACPH_INLINE static Quat		sFromTo(Vec3Arg inFrom, Vec3Arg inTo);
	...
	ACPH_INLINE float			LengthSq() const												{ return mValue.LengthSq(); }

	/// Length of quaternion.
	/// @return Length of quaternion (\f$|v|\f$)
	ACPH_INLINE float			Length() const													{ return mValue.Length(); }

	/// Normalize the quaternion (make it length 1)
	ACPH_INLINE Quat				Normalized() const												{ return Quat(mValue.Normalized()); }

	///@}
	///@name Additions / multiplications
	///@{

	ACPH_INLINE void				operator += (QuatArg inRHS)										{ mValue += inRHS.mValue; }
	ACPH_INLINE void				operator -= (QuatArg inRHS)										{ mValue -= inRHS.mValue; }
	ACPH_INLINE void				operator *= (float inValue)										{ mValue *= inValue; }
	ACPH_INLINE void				operator /= (float inValue)										{ mValue /= inValue; }
	ACPH_INLINE Quat				operator - () const												{ return Quat(-mValue); }
	ACPH_INLINE Quat				operator + (QuatArg inRHS) const								{ return Quat(mValue + inRHS.mValue); }
	ACPH_INLINE Quat				operator - (QuatArg inRHS) const								{ return Quat(mValue - inRHS.mValue); }
	ACPH_INLINE Quat				operator * (QuatArg inRHS) const;
	ACPH_INLINE Quat				operator * (float inValue) const								{ return Quat(mValue * inValue); }
	...
	ACPH_INLINE Vec3				operator * (Vec3Arg inValue) const;

	/// Multiply a quaternion with imaginary components and no real component (x, y, z, 0) with a quaternion
	static ACPH_INLINE Quat		sMultiplyImaginary(Vec3Arg inLHS, QuatArg inRHS);

	/// Rotate a vector by the inverse of this quaternion
	ACPH_INLINE Vec3				InverseRotate(Vec3Arg inValue) const;

	/// Rotate a the vector (1, 0, 0) with this quaternion
	ACPH_INLINE Vec3				RotateAxisX() const;

	/// Rotate a the vector (0, 1, 0) with this quaternion
	ACPH_INLINE Vec3				RotateAxisY() const;

	/// Rotate a the vector (0, 0, 1) with this quaternion
	ACPH_INLINE Vec3				RotateAxisZ() const;

	/// Dot product
	ACPH_INLINE float			Dot(QuatArg inRHS) const										{ return mValue.Dot(inRHS.mValue); }

	/// The conjugate [w, -x, -y, -z] is the same as the inverse for unit quaternions
	ACPH_INLINE Quat				Conjugated() const												{ return Quat(mValue.FlipSign<-1, -1, -1, 1>()); }

	/// Get inverse quaternion
	ACPH_INLINE Quat				Inversed() const												{ return Conjugated() / Length(); }
	...
	ACPH_INLINE Quat				EnsureWPositive() const											{ return Quat(Vec4::sXor(mValue, Vec4::sAnd(mValue.SplatW(), UVec4::sReplicate(0x80000000).ReinterpretAsFloat()))); }

	/// Get a quaternion that is perpendicular to this quaternion
	ACPH_INLINE Quat				GetPerpendicular() const										{ return Quat(mValue.Swizzle<SWIZZLE_Y, SWIZZLE_X, SWIZZLE_W, SWIZZLE_Z>().FlipSign<1, -1, 1, -1>()); }
	...
	ACPH_INLINE float			GetRotationAngle(Vec3Arg inAxis) const							{ return GetW() == 0.0f? ACPH_PI : 2.0f * ATan(GetXYZ().Dot(inAxis) / GetW()); }
	...
	ACPH_INLINE Quat				GetTwist(Vec3Arg inAxis) const;
	...
	ACPH_INLINE void				GetSwingTwist(Quat &outSwing, Quat &outTwist) const;
	...
	ACPH_INLINE Quat				LERP(QuatArg inDestination, float inFraction) const;
	...
	ACPH_INLINE Quat				SLERP(QuatArg inDestination, float inFraction) const;

	/// Load 3 floats from memory (X, Y and Z component and then calculates W) reads 32 bits extra which it doesn't use
	static ACPH_INLINE Quat		sLoadFloat3Unsafe(const Float3 &inV);
	...
	ACPH_INLINE uint32			CompressUnitQuat() const										{ return mValue.CompressUnitVector(); }

	/// Decompress a unit quaternion from a 32 bit value
	ACPH_INLINE static Quat		sDecompressUnitQuat(uint32 inValue)								{ return Quat(Vec4::sDecompressUnitVector(inValue)); }
