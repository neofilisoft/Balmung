...
<a href="https://balmungengine.com/discord"><img src="https://discordapp.com/api/guilds/437989205315158016/widget.png"/></a>

Balmung is a branded workspace fork built on top of Flax Engine. The core runtime/build system stays Flax-compatible, while the workspace surface, startup branding, and build wrappers are customized for Balmung.

Flax Engine is a high quality modern 3D game engine written in C++ and C#. From stunning graphics to powerful scripts, it's designed for fast workflow with many ready-to-use features waiting for you right now. To learn more see the website ([www.balmungengine.com](https://balmungengine.com)).
...
* [Homepage](https://balmungengine.com)
* [Dev Blog](https://balmungengine.com/blog)
* [Documentation](https://docs.balmungengine.com)
* [Forum](https://forum.balmungengine.com)
* [Roadmap](https://trello.com/b/NQjLXRCP/balmung-roadmap)

# Screenshots

![pbr-rendering](Development/Images/balmung-pic-2.jpg "PBR Rendering and Global Illumination")
![rendering](Development/Images/balmung-pic-1.jpg "Rendering")
![performance](Development/Images/balmung-pic-3.jpg "High Performance")
...
* Clone repo (with LFS)
* Run **GenerateBalmungProjectFiles.bat** (or `GenerateProjectFiles.bat` if you want the upstream script directly)
* Open `Balmung.sln` and set solution configuration to **Editor.Development** and solution platform to **Win64**
* Set Balmung/Flax (C++) or FlaxEngine (C#) as startup project
* Compile the Balmung fork project (hit F7 or CTRL+Shift+B)
* Optionally set Debug Type to **Managed Only (.NET Core)** to debug C#-only, or **Mixed (.NET Core)** to debug both C++ and C#
* Run `BuildBalmungEditor.bat` if you want a ready-to-launch `BalmungEditor.exe` alias in `Binaries\Editor\Win64\Development`

### Balmung build shortcuts

* Generate project files: `GenerateBalmungProjectFiles.bat`
* Build a ready-to-run editor exe alias: `BuildBalmungEditor.bat`
* Direct build command: `Development\Scripts\Windows\CallBuildTool.bat -build -platform=Windows -arch=x64 -configuration=Development -buildtargets=BalmungEditor`
* Package editor binaries: `PackageBalmungEditor.bat`
...
* Clone the repository (with LFS)
  * git-lfs clone https://github.com/FlaxEngine/BalmungEngine.git
...
- **Cache/** - local data cache folder used by the engine and tools
  - **Intermediate/** - intermediate files and cache for engine build
    ...
    - **Deps/** - Balmung.Build dependencies building cache
